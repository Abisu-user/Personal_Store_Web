...
const manager = new StorageManager(new R2StorageProvider({
...
const directBytes = new TextEncoder().encode("Personal Vault R2 direct validation");
const signedBytes = new TextEncoder().encode("Personal Vault R2 signed validation");
const created = [];
...
if (cleanupErrors.length) throw new Error(`R2 cleanup failed: ${cleanupErrors.join("; ")}`);
if (validationError) throw validationError;
console.log(JSON.stringify({ ok: true, prefix: "test/phase-3/<uuid>", directPut: true, signedPut: true, head: true, get: true, signedGet: true, cleanup: "verified" }));
