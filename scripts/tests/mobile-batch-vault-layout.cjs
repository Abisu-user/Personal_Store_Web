...
async function main() {
  ...
  try {
    ...
    for (const width of [375, 390, 430]) {
      ...
      await page.goto(base); await page.locator(".batch-action-bar").evaluate(el => Promise.all(el.getAnimations().map(animation => animation.finished)));
      const bar = await page.locator(".batch-action-bar").boundingBox(), nav = await page.locator(".mobile-bottom-nav").boundingBox();
      assert.ok(bar.x >= 0 && bar.x + bar.width <= width && bar.y + bar.height <= nav.y - 7.5, `batch geometry ${width}`);
      for (const button of await page.locator(".batch-action-bar button").all()) assert.ok((await button.boundingBox()).height >= 44);
      ...
      const safeBar = await page.locator(".batch-action-bar").boundingBox(), safeNav = await page.locator(".mobile-bottom-nav").boundingBox();
      ...
      assert.equal(await page.locator(".batch-action-bar").evaluate(el => getComputedStyle(el).pointerEvents), "none");
      await page.screenshot({ path: path.join(out, `vault-batch-${width}.png`), animations: "disabled" });
      report.push({ width, batchHeight: bar.height, safeArea: "34px pass" });
    }
    for (const width of [1366, 1440, 1920]) {
      await page.setViewportSize({ width, height: 900 }); await page.goto(base);
      const bar = await page.locator(".batch-action-bar").boundingBox();
      assert.ok(bar.x >= 0 && bar.x + bar.width <= width && bar.y + bar.height <= 900, `desktop batch geometry ${width}`);
    }
