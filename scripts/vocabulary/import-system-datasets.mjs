    ...
    for (const [batchNumber, batch] of chunks(entries).entries()) {
      ...
      const dictionaryRows = batch.map((entry) => {
        ...
        return {
          ...
          source_metadata: { ...(existing?.source_metadata ?? {}), jlptLevel: entry.level, kanaGroup: entry.kanaGroup, ...(verified ? { traditionalChineseTranslationSource: verified.translationMetadata?.source ?? "tomoshi-jmdict-zhtw", traditionalChineseTranslationConfidence: verified.translationMetadata?.confidence ?? "source-normalized", traditionalChineseTranslationPolicy: verified.translationMetadata?.sensePolicy ?? "source-order-with-metadata-filter", traditionalChineseVerified: true } : {}) },
        };
      });
