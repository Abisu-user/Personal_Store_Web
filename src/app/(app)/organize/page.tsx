...
export default async function OrganizePage({ searchParams }: { searchParams: Promise<{ type?: string }> }) {
  ...
  return <main className="dashboard"><section className="dashboard-card"><p className="eyebrow">ORGANIZE YOUR VAULT</p><h1>管理類別／資料夾</h1><p>集中管理收藏、筆記、程式碼、檔案與照片的整理方式；分頁切換後只會修改該類型的資料夾。</p><CategoryFolderManagement bookmarks={bookmarks} code={code} files={files} initialTab={initialTab} notes={notes} photos={photos} /></section></main>;
}
