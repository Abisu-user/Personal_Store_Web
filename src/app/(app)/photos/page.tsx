...
export default async function PhotosPage() {
  ...
  return <main className={`dashboard ${collectionStyles.collectionPage} ${mobileStyles.photosPage}`}><section className="dashboard-card"><MobilePageHeader eyebrow="PHOTOS" title="照片" actions={<CreateItemButton className="mobile-header-create-button" kind="photo"><AppIcon name="plus" /><span className="sr-only">上傳照片</span></CreateItemButton>} /><div className="page-heading"><div><p className="eyebrow">PRIVATE PHOTO STORAGE</p><h1>照片</h1><p>將照片保存在私有空間；可用類別、資料夾與常駐清單整理，刪除後會保留在垃圾桶 30 天。</p></div><CreateItemButton kind="photo"><span className={mobileStyles.desktopAddLabel}>＋ 上傳照片</span><span className={mobileStyles.mobileAddLabel}><AppIcon name="plus" />上傳</span></CreateItemButton></div><PhotosWorkspace initialData={await getPhotosWorkspaceData(user.id)} /></section></main>;
}
