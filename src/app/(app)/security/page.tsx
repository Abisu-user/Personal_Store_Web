...
export default async function SecurityPage() {
  ...
  return <main className="dashboard"><section className="dashboard-card"><p className="eyebrow">ACCOUNT SECURITY</p><h1>裝置與安全活動</h1><p>管理已登入的裝置，並查看服務端記錄的安全事件。</p><p><Link className="text-link" href="/security/mfa">管理雙因素驗證</Link></p><PasskeySettings />{permissions.adultContentAccess && <AdultContentSettings canAccess />}{permissions.adultContentAdmin && <AdultPermissionManager />}<FolderLockSecuritySettings /><SecurityActivity /></section></main>;
}
