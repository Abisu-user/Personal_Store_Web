...
const bahamutUrl = z.string().trim().url().max(1000).refine((value) => new URL(value).protocol === "https:" && new URL(value).hostname === "ani.gamer.com.tw", "請使用動畫瘋官方 HTTPS 連結。").nullable().optional();
const createSchema = z.object({ externalId: z.string().regex(/^\d{1,12}$/), externalSource: z.enum(["jikan", "anilist", "bangumi"]).default("jikan"), watchStatus: status.default("planning"), watchedEpisodes: z.number().int().min(0).max(100000).default(0), tagIds: z.array(id).max(30).default([]), favorite: z.boolean().default(false), bahamutUrl });
const updateSchema = z.object({ id, watchStatus: status.optional(), watchedEpisodes: z.number().int().min(0).max(100000).optional(), rating: z.number().min(0).max(10).nullable().optional(), favorite: z.boolean().optional(), personalRank: rank.nullable().optional(), notes: z.string().trim().max(12000).nullable().optional(), tagIds: z.array(id).max(30).optional(), bahamutUrl });
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { data, error: insertError } = await admin.from("anime_library").upsert({ user_id: context.userId, ...rowFor(anime, parsed.data.bahamutUrl), watch_status: isCompleted ? "completed" : parsed.data.watchStatus, watched_episodes: parsed.data.watchedEpisodes, favorite: parsed.data.favorite, started_watching_at: parsed.data.watchStatus === "watching" ? new Date().toISOString().slice(0, 10) : null, completed_at: isCompleted || parsed.data.watchStatus === "completed" ? new Date().toISOString().slice(0, 10) : null }, { onConflict: "user_id,external_source,external_id" }).select("id").single();
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    const { data: current, error: currentError } = await admin.from("anime_library").select("episodes,watched_episodes,watch_status,title").eq("id", animeId).eq("user_id", context.userId).is("deleted_at", null).maybeSingle();
    ...
    const updates: Record<string, unknown> = {}; if (changes.watchStatus) updates.watch_status = changes.watchStatus; if (changes.watchedEpisodes !== undefined) updates.watched_episodes = changes.watchedEpisodes; if (changes.rating !== undefined) updates.rating = changes.rating; if (changes.favorite !== undefined) updates.favorite = changes.favorite; if (changes.personalRank !== undefined) updates.personal_rank = changes.personalRank; if (changes.notes !== undefined) updates.notes = changes.notes || null; if (changes.bahamutUrl !== undefined) Object.assign(updates, changes.bahamutUrl ? { bahamut_available: true, bahamut_url: changes.bahamutUrl, bahamut_title: current.title, bahamut_sn: null, bahamut_last_checked_at: new Date().toISOString() } : { bahamut_available: null, bahamut_url: null, bahamut_title: null, bahamut_sn: null, bahamut_last_checked_at: null }); if (changes.watchStatus === "watching" && current.watch_status !== "watching") updates.started_watching_at = new Date().toISOString().slice(0, 10); if (changes.watchStatus === "completed" || completed) { updates.watch_status = "completed"; updates.completed_at = new Date().toISOString().slice(0, 10); }
