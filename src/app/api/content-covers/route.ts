...
export async function GET(request: NextRequest) {
  ...
  const { data: signed, error: signedError } = await admin.storage.from("content-covers").createSignedUrl(data.cover_image_path, 60 * 60);
  if (signedError || !signed?.signedUrl) return NextResponse.json({ error: "暫時無法讀取封面。" }, { status: 503 });
