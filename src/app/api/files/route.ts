...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const tags = await resolveTags(context.userId, parsed.data.tags); const { data: entry, error: entryError } = await admin.from("entries").insert({ owner_id: context.userId, kind: "file", title: parsed.data.title, description: parsed.data.description || null, category_id: parsed.data.categoryId ?? null, content_folder_id: parsed.data.contentFolderId ?? null, is_favorite: parsed.data.favorite, is_pinned: parsed.data.pinned && !parsed.data.archived, is_archived: parsed.data.archived, ...entryCoverFields(cover) }).select("id").single(); if (entryError) throw entryError;
