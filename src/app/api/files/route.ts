...
export async function PATCH(request: NextRequest) {
  ...
  if (!(await validateCategory(context.userId, parsed.data.categoryId)) || !(await validateContentFolder(context.userId, "file", parsed.data.contentFolderId))) return jsonError("找不到指定分類或資料夾。", 400);
