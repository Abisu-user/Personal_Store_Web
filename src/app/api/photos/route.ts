...
async function validateCategory(ownerId: string, categoryId: string | null | undefined) {
  ...
  const { data } = await createAdminClient().from("categories").select("id").eq("id", categoryId).eq("owner_id", ownerId).eq("content_kind", "photo").maybeSingle();
  return Boolean(data);
}
...
export async function POST(request: NextRequest) {
  ...
  if (!(await validateCategory(context.userId, parsed.data.categoryId)) || !(await validateContentFolder(context.userId, "photo", parsed.data.contentFolderId))) return error("找不到指定分類或資料夾。", 400);
