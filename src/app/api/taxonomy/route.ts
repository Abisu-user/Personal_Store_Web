...
const inputSchema = z.object({ kind: taxonomyKind, name: z.string().trim().min(1).max(80) });
const deleteSchema = z.object({ kind: taxonomyKind, id: z.string().uuid() });
const updateSchema = z.object({ kind: taxonomyKind, id: z.string().uuid(), name: z.string().trim().min(1).max(80).optional(), visible: z.boolean().optional() }).refine((value) => value.name !== undefined || value.visible !== undefined);
...
export async function POST(request: NextRequest) {
  ...
  try {
    const admin = createAdminClient(); const table = tableFor(parsed.data.kind);
    const { data, error } = await admin.from(table).insert({ owner_id: context.userId, name: parsed.data.name }).select(selectFor(parsed.data.kind)).single();
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    const { data, error } = await admin.from(table).update(updates).eq("id", parsed.data.id).eq("owner_id", context.userId).select("id").maybeSingle();
    if (error?.code === "23505") return response("此名稱已存在。", 409);
    if (error) throw error; if (!data) return response("Not found", 404);
