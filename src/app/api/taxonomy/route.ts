...
export async function PATCH(request: NextRequest) {
  const context = await getSecurityContext(); if (!context) return response("Unauthorized", 401);
  const parsed = updateSchema.safeParse(await request.json().catch(() => null)); if (!parsed.success) return response("請輸入有效名稱。", 400);
  try {
    const admin = createAdminClient(); const table = parsed.data.kind === "category" ? "categories" : "tags";
    const { data, error } = await admin.from(table).update({ name: parsed.data.name }).eq("id", parsed.data.id).eq("owner_id", context.userId).select("id").maybeSingle();
    if (error?.code === "23505") return response("此名稱已存在。", 409);
    if (error) throw error; if (!data) return response("Not found", 404);
    ...
    return NextResponse.json({ ok: true });
  } catch { return response("無法更新分類或標籤。", 503); }
}
