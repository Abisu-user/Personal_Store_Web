...
import { readEntryTaxonomyLinks, replaceEntryTaxonomy, validateEntryTaxonomy } from "@/lib/content/entry-taxonomy";

const base64 = z.string().min(1).max(200_000).regex(/^[A-Za-z0-9+/]+={0,2}$/);
const itemSchema = z.object({ id: z.string().uuid(), ciphertext: base64, nonce: base64, aad: z.object({ entryId: z.string().uuid(), version: z.literal(1) }), categoryId: z.string().uuid().nullable().optional(), categoryIds: z.array(z.string().uuid()).max(30).optional() });
...
const categoryIdsFor = (item: { categoryId?: string | null; categoryIds?: string[] }) => [...new Set(item.categoryIds ?? (item.categoryId ? [item.categoryId] : []))];
...
export async function GET() {
  ...
  try {
    ...
    const links = await readEntryTaxonomyLinks(context.userId, "content");
    const items = (data ?? []).flatMap((entry) => { const payload = Array.isArray(entry.vault_payloads) ? entry.vault_payloads[0] : entry.vault_payloads; if (!payload) return []; const categoryIds = links.get(entry.id)?.categories.map((category) => category.id) ?? (entry.category_id ? [entry.category_id] : []); return [{ id: entry.id, categoryId: categoryIds[0] ?? null, categoryIds, ciphertext: asBase64(payload.ciphertext), nonce: asBase64(payload.nonce), aad: payload.aad, encryptionVersion: payload.encryption_version }]; });
...
export async function POST(request: NextRequest) {
  ...
  try {
    const categoryIds = categoryIdsFor(item); const admin = createAdminClient(); const { data: vault } = await admin.from("vaults").select("owner_id").eq("owner_id", context.userId).maybeSingle(); if (!vault) return NextResponse.json({ error: "請先建立 Vault。" }, { status: 409 }); if (!(await validateEntryTaxonomy(context.userId, "vault_item", categoryIds, [], "content"))) return NextResponse.json({ error: "找不到指定的保管庫分類。" }, { status: 400 }); const { error: entryError } = await admin.from("entries").insert({ id: item.id, owner_id: context.userId, kind: "vault_item", security_level: "vault", title: "Private Vault record", description: null, category_id: categoryIds[0] ?? null }); if (entryError) throw entryError;
    ...
    try { await replaceEntryTaxonomy(context.userId, item.id, "vault_item", categoryIds, [], "content"); } catch (cause) { await admin.from("entries").delete().eq("id", item.id).eq("owner_id", context.userId); throw cause; }
...
export async function PATCH(request: NextRequest) {
  ...
  const item = parseItemPayload(await request.json().catch(() => null)); if (!item) return NextResponse.json({ error: "加密資料無效。" }, { status: 400 });
  try {
    const categoryIds = categoryIdsFor(item); if (!(await validateEntryTaxonomy(context.userId, "vault_item", categoryIds, [], "content"))) return NextResponse.json({ error: "找不到指定的保管庫分類。" }, { status: 400 });
    ...
    try { await replaceEntryTaxonomy(context.userId, item.id, "vault_item", categoryIds, [], "content"); } catch (cause) { await admin.from("vault_payloads").update(previousPayload).eq("entry_id", item.id); throw cause; }
