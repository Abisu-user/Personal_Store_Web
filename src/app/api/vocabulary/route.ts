...
const exampleSchema = z.object({ id, meaningId: id.optional().nullable(), sentence: z.string().trim().min(1).max(3000), reading: z.string().trim().max(1000).optional().nullable(), translation: z.string().trim().max(3000).optional().nullable(), translationZhTw: z.string().trim().max(3000).optional().nullable(), source: z.string().trim().max(500).optional().nullable(), notes: z.string().trim().max(3000).optional().nullable(), isFavorite: z.boolean().default(false) }).partial({ id: true });
...
async function replaceRelations(cardId: string, payload: z.infer<typeof payloadSchema>) {
  ...
  if (payload.examples.length) { const { error } = await admin.from("vocabulary_examples").insert(payload.examples.map((example) => ({ card_id: cardId, meaning_id: example.meaningId && meanings.some((meaning) => meaning.id === example.meaningId) ? example.meaningId : null, sentence: example.sentence, reading: emptyToNull(example.reading), translation: emptyToNull(example.translation), translation_zh_tw: payload.language === "ja" ? emptyToNull(example.translationZhTw || example.translation) : null, source: emptyToNull(example.source), notes: emptyToNull(example.notes), is_favorite: example.isFavorite }))); if (error) throw error; }
...
export async function GET(request: NextRequest) { const context = await getSecurityContext(); if (!context) return jsonError("Unauthorized", 401); const requestedLanguage = request.nextUrl.searchParams.get("language"); if (requestedLanguage && requestedLanguage !== "ja" && requestedLanguage !== "en") return jsonError("language 僅能是 ja 或 en。", 400); try { return NextResponse.json(await getVocabularyWorkspaceData(context.userId, request.nextUrl.searchParams.get("trash") === "1", requestedLanguage as "ja" | "en" | undefined), { headers: { "Cache-Control": "private, no-store" } }); } catch { return jsonError("單字資料暫時無法讀取。", 503); } }
