...
export function AnimeWorkspace({ initialData }: { initialData?: AnimeWorkspaceData }) {
  ...
  return <section className="anime-workspace">
    ...
    {tab === "adult" && adultUnlocked && adultData && <section className="anime-adult-workspace">
      ...
      {adultView === "library" ? <>
        <AnimeFolderNavigation folders={adultData.folders} onChange={(folderId) => { setAdultFolderFilter(folderId); setAdultCategoryFilter(null); }} onFoldersChange={(folders) => setAdultData((current) => current ? { ...current, folders } : current)} scope="adult" selectedId={adultFolderFilter} />
    ...
    {tab === "library" && <>
      ...
      <AnimeFolderNavigation folders={data.folders} onChange={(folderId) => { setFolderFilter(folderId); setCategoryFilter(null); }} onFoldersChange={(folders) => setData((current) => ({ ...current, folders }))} scope="standard" selectedId={folderFilter} />
      ...
      {!loaded ? <AnimeGridSkeleton /> : <div className="anime-grid">{pagedLibrary.map((anime) => <article className="anime-card" key={anime.id}>
