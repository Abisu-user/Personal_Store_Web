...
const empty: AnimeWorkspaceData = { library: [], tags: [], folders: [], logs: [], preferences: defaultPreferences };
...
function AnimeFolderNavigation({ folders, onChange, onFoldersChange, scope, selectedId, showUnorganized = true }: { folders: AnimeWorkspaceData["folders"]; onChange: (folderId: string | null) => void; onFoldersChange: (folders: AnimeWorkspaceData["folders"]) => void; scope: CategoryScope; selectedId: string | null; showUnorganized?: boolean }) {
  const [adding, setAdding] = useState(false); const [managing, setManaging] = useState(false); const [name, setName] = useState(""); const [pending, setPending] = useState(false); const [error, setError] = useState<string | null>(null);
  const visible = folders.filter((folder) => folder.isVisible);
  const create = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); if (!name.trim()) return; setPending(true); setError(null); try { const result = await api<{ folder: AnimeWorkspaceData["folders"][number] }>("/api/anime/folders", { method: "POST", body: JSON.stringify({ name: name.trim(), scope }) }); onFoldersChange([...folders, result.folder].sort((a, b) => a.sortOrder - b.sortOrder)); setName(""); setAdding(false); } catch (cause) { setError(cause instanceof Error ? cause.message : "無法新增資料夾。"); } finally { setPending(false); } };
  const update = async (folder: AnimeWorkspaceData["folders"][number], changes: Record<string, unknown>) => { setPending(true); setError(null); try { const result = await api<{ folder: AnimeWorkspaceData["folders"][number] }>("/api/anime/folders", { method: "PATCH", body: JSON.stringify({ id: folder.id, ...changes }) }); onFoldersChange(folders.map((item) => item.id === folder.id ? result.folder : item)); } catch (cause) { setError(cause instanceof Error ? cause.message : "無法修改資料夾。"); } finally { setPending(false); } };
  const remove = async (folder: AnimeWorkspaceData["folders"][number]) => { if (!window.confirm(`移除「${folder.name}」？其中的動漫會回到未整理。`)) return; setPending(true); setError(null); try { await api("/api/anime/folders", { method: "DELETE", body: JSON.stringify({ id: folder.id }) }); onFoldersChange(folders.filter((item) => item.id !== folder.id)); if (selectedId === folder.id) onChange(null); } catch (cause) { setError(cause instanceof Error ? cause.message : "無法移除資料夾。"); } finally { setPending(false); } };
  return <section className="anime-folder-bar" aria-label="動漫資料夾"><div className="anime-category-scroll">{showUnorganized && <button className={!selectedId ? "active" : ""} onClick={() => onChange(null)} type="button">未整理</button>}{visible.map((folder) => <button className={selectedId === folder.id ? "active" : ""} key={folder.id} onClick={() => onChange(folder.id)} type="button">{folder.name}</button>)}<button aria-label="新增資料夾" className="anime-category-utility anime-category-add-button" onClick={() => setAdding(true)} type="button">＋</button><button aria-label="管理資料夾" className="anime-category-utility" onClick={() => setManaging(true)} type="button">更多</button></div><ModalDialog className="mobile-sheet-dialog" onClose={() => setAdding(false)} open={adding} pending={pending} title="新增動漫資料夾"><form className="anime-category-dialog" onSubmit={create}><label>資料夾名稱<input autoFocus disabled={pending} onChange={(event) => setName(event.target.value)} placeholder="例如：本季、經典作品" value={name} /></label>{error && <p className="notice error">{error}</p>}<div className="dialog-actions"><button className="secondary-button" disabled={pending} onClick={() => setAdding(false)} type="button">取消</button><button className="button" disabled={pending || !name.trim()} type="submit">新增資料夾</button></div></form></ModalDialog><ModalDialog className="mobile-sheet-dialog" onClose={() => setManaging(false)} open={managing} pending={pending} title="管理動漫資料夾"><div className="anime-category-manager-list">{folders.length ? folders.map((folder) => <div className="anime-folder-manager-row" key={folder.id}><button onClick={() => { onChange(folder.id); setManaging(false); }} type="button">{folder.name}{folder.isVisible ? "" : "（已隱藏）"}</button><div><button className="secondary-button compact" disabled={pending} onClick={() => { const next = window.prompt("資料夾名稱", folder.name)?.trim(); if (next && next !== folder.name) void update(folder, { name: next }); }} type="button">修改</button><button className="secondary-button compact" disabled={pending} onClick={() => void update(folder, { isVisible: !folder.isVisible })} type="button">{folder.isVisible ? "隱藏" : "顯示"}</button><button className="delete-button compact" disabled={pending} onClick={() => void remove(folder)} type="button">移除</button></div></div>) : <p className="anime-field-hint">尚未建立資料夾。</p>}</div>{error && <p className="notice error">{error}</p>}</ModalDialog></section>;
}

function AnimeCollectionList({ adult = false, blur = false, categories, folders, items, onMutated, onOpen, scope, trashed = false }: { adult?: boolean; blur?: boolean; categories: AnimeTag[]; folders: AnimeWorkspaceData["folders"]; items: AnimeLibraryItem[]; onMutated: () => Promise<void>; onOpen: (anime: AnimeLibraryItem) => void; scope: CategoryScope; trashed?: boolean }) {
  const [selecting, setSelecting] = useState(false); const [selectedIds, setSelectedIds] = useState<string[]>([]); const [organizeOpen, setOrganizeOpen] = useState(false); const [folderId, setFolderId] = useState(""); const [categoryIds, setCategoryIds] = useState<string[]>([]); const [confirmPermanent, setConfirmPermanent] = useState(false); const [pending, setPending] = useState(false); const [message, setMessage] = useState<string | null>(null);
  const selected = new Set(selectedIds); const allSelected = items.length > 0 && selectedIds.length === items.length;
  const toggle = (animeId: string) => setSelectedIds((ids) => ids.includes(animeId) ? ids.filter((id) => id !== animeId) : [...ids, animeId]);
  const run = async (action: "trash" | "restore" | "permanent" | "organize") => { if (!selectedIds.length) return; setPending(true); setMessage(null); try { await api("/api/anime/library", { method: "PATCH", body: JSON.stringify({ action, ids: selectedIds, scope, ...(action === "organize" ? { folderId: folderId || null, categoryIds } : {}) }) }); await onMutated(); setSelectedIds([]); setSelecting(false); setOrganizeOpen(false); setConfirmPermanent(false); } catch (cause) { setMessage(cause instanceof Error ? cause.message : "無法完成批量操作。"); } finally { setPending(false); } };
  return <>
    {pending && <OperationStatus label="正在處理選取的動漫…" />}
    <div className="anime-bulk-toolbar">
      <button className="secondary-button compact" onClick={() => { setSelecting((value) => !value); setSelectedIds([]); }} type="button">{selecting ? "取消選取" : "選取"}</button>
      {selecting && <><strong>已選 {selectedIds.length} 筆</strong><button className="secondary-button compact" onClick={() => setSelectedIds(allSelected ? [] : items.map((anime) => anime.id))} type="button">{allSelected ? "取消全選" : "全選"}</button>{trashed ? <><button className="button compact" disabled={!selectedIds.length || pending} onClick={() => void run("restore")} type="button">還原</button><button className="danger-button compact" disabled={!selectedIds.length || pending} onClick={() => setConfirmPermanent(true)} type="button">永久刪除</button></> : <><button className="secondary-button compact" disabled={!selectedIds.length || pending} onClick={() => setOrganizeOpen(true)} type="button">整理</button><button className="danger-button compact" disabled={!selectedIds.length || pending} onClick={() => void run("trash")} type="button">移至垃圾桶</button></>}</>}
    </div>
    {message && <p className="notice error">{message}</p>}
    <div className="anime-grid">{items.map((anime) => <article className={`anime-card${adult ? " anime-adult-card" : ""}`} key={anime.id}>{selecting && <label className="anime-card-select"><input aria-label={`選取 ${displayTitle(anime)}`} checked={selected.has(anime.id)} onChange={() => toggle(anime.id)} type="checkbox" /></label>}<button className="anime-card-main" onClick={() => onOpen(anime)} type="button"><Cover anime={anime} blur={adult && blur} /><div className="anime-card-copy"><div className="anime-card-line"><Status value={anime.watchStatus} />{adult && <span className="anime-adult-badge">18+</span>}{anime.rating !== null && !adult && <span className="anime-rating-summary">★ {anime.rating}</span>}</div><h3>{displayTitle(anime)}</h3><p>{anime.tags.map((category) => category.name).join(" · ") || "未分類"}</p>{!adult && <div className="anime-card-link">{anime.sourceUrl ? "已設定觀看連結" : "尚未設定觀看連結"}</div>}</div></button></article>)}</div>
    <ModalDialog className="mobile-sheet-dialog" onClose={() => setOrganizeOpen(false)} open={organizeOpen} pending={pending} title="批量整理動漫"><div className="anime-category-dialog"><label>移至資料夾<select onChange={(event) => { setFolderId(event.target.value); setCategoryIds([]); }} value={folderId}><option value="">未整理</option>{folders.filter((folder) => folder.isVisible || folder.id === folderId).map((folder) => <option key={folder.id} value={folder.id}>{folder.name}</option>)}</select></label><fieldset><legend>類別（可複選）</legend><div className="anime-tag-picker">{categories.filter((category) => category.folderId === (folderId || null)).length ? categories.filter((category) => category.folderId === (folderId || null)).map((category) => <label key={category.id}><input checked={categoryIds.includes(category.id)} onChange={() => setCategoryIds((ids) => ids.includes(category.id) ? ids.filter((id) => id !== category.id) : [...ids, category.id])} type="checkbox" /> {category.name}</label>) : <p className="anime-field-hint">此資料夾尚未建立類別。</p>}</div></fieldset><div className="dialog-actions"><button className="secondary-button" disabled={pending} onClick={() => setOrganizeOpen(false)} type="button">取消</button><button className="button" disabled={pending} onClick={() => void run("organize")} type="button">套用整理</button></div></div></ModalDialog>
    <ConfirmDialog confirmLabel="永久刪除" description={`確定要永久刪除選取的 ${selectedIds.length} 筆動漫嗎？此操作無法復原。`} onCancel={() => setConfirmPermanent(false)} onConfirm={() => void run("permanent")} open={confirmPermanent} pending={pending} title="永久刪除動漫" />
  </>;
}
...
export function AnimeWorkspace({ initialData }: { initialData?: AnimeWorkspaceData }) {
  ...
  const [adultFolderFilter, setAdultFolderFilter] = useState<string | null>(null);
  ...
  const [adultTrashData, setAdultTrashData] = useState<AnimeWorkspaceData | null>(null);
  const [libraryView, setLibraryView] = useState<"library" | "trash">("library");
  const [adultLibraryView, setAdultLibraryView] = useState<"library" | "trash">("library");
  ...
  useEffect(() => {
    ...
    if (cached) { setData(cached); setPreferences(cached.preferences); setLoaded(true); }
    const load = async () => {
      try {
        const next = await api<AnimeWorkspaceData>("/api/anime/library");
        if (!active) return;
        setData(next); setPreferences(next.preferences); setLoaded(true); writeClientResource("anime:standard", next);
      } catch (cause) {
        if (active && !cached) setNotice(cause instanceof Error ? cause.message : "無法載入動漫資料，請稍後再試。");
      }
    };
    void load();
    return () => { active = false; };
  ...
  useEffect(() => { if (loaded) writeClientResource("anime:standard", data); }, [data, loaded]);
  ...
  const refreshTrash = async (scope: CategoryScope) => {
    const next = await api<AnimeWorkspaceData>(`/api/anime/library?view=trash${scope === "adult" ? "&scope=adult" : ""}`);
    if (scope === "adult") setAdultTrashData(next); else setTrashData(next);
  };
  const openTrash = async (scope: CategoryScope) => {
    setPending("trash");
    try { await refreshTrash(scope); if (scope === "adult") setAdultLibraryView("trash"); else setLibraryView("trash"); }
    catch (cause) { setNotice(cause instanceof Error ? cause.message : "無法讀取垃圾桶。"); }
  ...
  const library = useMemo(() => data.library.filter((anime) => {
    ...
    const matchesFolder = folderFilter ? anime.folderId === folderFilter : !anime.folderId;
    ...
    return matchesFilter && matchesFolder && matchesCategory && haystack.includes(query.toLocaleLowerCase());
  ...
  useEffect(() => { setLibraryPage(1); }, [filter, categoryFilter, folderFilter, query]);
  ...
  const createCategory = async (scope: CategoryScope) => {
    ...
    try {
      ...
      const answer = await api<{ tag: AnimeTag }>("/api/anime/tags", { method: "POST", body: JSON.stringify({ name, scope, folderId }) });
  ...
  return <section className="anime-workspace">
    ...
    {tab === "adult" && adultUnlocked && adultData && <section className="anime-adult-workspace">
      ...
      {adultView === "library" ? <>
        <div className="anime-filter-bar anime-adult-filter-bar"><input aria-label="搜尋成人動漫" onChange={(event) => setAdultQuery(event.target.value)} placeholder="搜尋成人動漫" value={adultQuery} /><button className={!adultFolderFilter ? "active secondary-button compact" : "secondary-button compact"} onClick={() => { setAdultFolderFilter(null); setAdultCategoryFilter(null); }} type="button">未整理</button></div>
        <AnimeFolderNavigation folders={adultData.folders} onChange={(folderId) => { setAdultLibraryView("library"); setAdultFolderFilter(folderId); setAdultCategoryFilter(null); }} onFoldersChange={(folders) => setAdultData((current) => current ? { ...current, folders } : current)} scope="adult" selectedId={adultFolderFilter} showUnorganized={false} />
        <div className="anime-bulk-toolbar"><button className={adultLibraryView === "trash" ? "active secondary-button compact" : "secondary-button compact"} onClick={() => void openTrash("adult")} type="button">垃圾桶{adultTrashData ? ` ${adultTrashData.library.length}` : ""}</button>{adultLibraryView === "trash" && <button className="secondary-button compact" onClick={() => setAdultLibraryView("library")} type="button">返回我的動漫</button>}</div>
        {adultLibraryView === "library" ? <><section className="anime-category-bar anime-adult-category-bar" aria-label="成人動漫類別"><div className="anime-category-scroll"><button className={!adultCategoryFilter ? "active" : ""} onClick={() => setAdultCategoryFilter(null)} type="button">所有類別</button>{adultData.tags.filter((category) => category.folderId === adultFolderFilter).map((category) => <button className={adultCategoryFilter === category.id ? "active" : ""} key={category.id} onClick={() => setAdultCategoryFilter(category.id)} type="button">{category.name} <small>{adultData.library.filter((anime) => anime.tags.some((item) => item.id === category.id)).length}</small></button>)}<button aria-label="查看更多成人動漫類別" className="anime-category-utility" onClick={() => setCategoryMoreOpen(true)} type="button">更多</button><button aria-label="新增成人動漫類別" className="anime-category-utility anime-category-add-button" onClick={() => setCategoryAddOpen(true)} type="button">＋</button></div></section><AnimeCollectionList adult blur={preferences.blurAdultCovers} categories={adultData.tags} folders={adultData.folders} items={adultData.library.filter((anime) => (adultFolderFilter ? anime.folderId === adultFolderFilter : !anime.folderId) && (!adultCategoryFilter || anime.tags.some((category) => category.id === adultCategoryFilter)) && `${displayTitle(anime)} ${anime.notes ?? ""} ${anime.tags.map((tag) => tag.name).join(" ")}`.toLocaleLowerCase().includes(adultQuery.trim().toLocaleLowerCase()))} onMutated={async () => { await Promise.all([refreshAdult(), refreshTrash("adult")]); }} onOpen={(anime) => { setSelectedReadOnly(false); setSelected(anime); }} scope="adult" /></> : <AnimeCollectionList adult blur={preferences.blurAdultCovers} categories={adultTrashData?.tags ?? adultData.tags} folders={adultData.folders} items={adultTrashData?.library ?? []} onMutated={async () => { await Promise.all([refreshAdult(), refreshTrash("adult")]); }} onOpen={(anime) => { setSelectedReadOnly(true); setSelected(anime); }} scope="adult" trashed />}
        {adultLibraryView === "library" && !adultData.library.length && <div className="anime-empty"><h3>尚未新增成人作品</h3><p>可使用上方按鈕自行記錄成人作品與觀看連結。</p></div>}
      </> : <AnimeDiscovery adultMode library={adultData.library} onAdd={setAdultPrefill} />}
    </section>}
    {tab === "adult" && adultUnlocked && adultData && <><ModalDialog className="mobile-sheet-dialog" onClose={() => setCategoryAddOpen(false)} open={categoryAddOpen} pending={pending === "category"} title="新增成人動漫類別"><form className="anime-category-dialog" onSubmit={(event) => { event.preventDefault(); void createCategory("adult"); }}><label>類別名稱<input autoFocus disabled={Boolean(pending)} onChange={(event) => setCategoryName(event.target.value)} placeholder="例如：收藏、系列" value={categoryName} /></label><div className="dialog-actions"><button className="secondary-button" disabled={Boolean(pending)} onClick={() => setCategoryAddOpen(false)} type="button">取消</button><button className="button" disabled={Boolean(pending) || !categoryName.trim()} type="submit">新增類別</button></div></form></ModalDialog><ModalDialog className="mobile-sheet-dialog" onClose={() => setCategoryMoreOpen(false)} open={categoryMoreOpen} title="成人動漫類別"><div className="anime-category-dialog"><p>選擇目前資料夾的類別以篩選成人作品。</p><input aria-label="搜尋成人動漫類別" onChange={(event) => setCategoryQuery(event.target.value)} placeholder="搜尋類別" value={categoryQuery} /><div className="anime-category-manager-list"><button className={!adultCategoryFilter ? "active" : ""} onClick={() => { setAdultCategoryFilter(null); setCategoryMoreOpen(false); }} type="button">所有類別</button>{adultData.tags.filter((item) => item.folderId === adultFolderFilter && item.name.toLocaleLowerCase().includes(categoryQuery.trim().toLocaleLowerCase())).map((item) => <button className={adultCategoryFilter === item.id ? "active" : ""} key={item.id} onClick={() => { setAdultCategoryFilter(item.id); setCategoryMoreOpen(false); }} type="button">{item.name} <small>{adultData.library.filter((anime) => anime.tags.some((tag) => tag.id === item.id)).length}</small></button>)}</div></div></ModalDialog></>}
    {tab === "library" && <>
      <div className="anime-filter-bar">
        <div className="anime-filter-scroll">{visibleFilters.map((value) => <button className={filter === value ? "active" : ""} key={value} onClick={() => setFilter(value)} type="button">{value === "all" ? "全部" : animeStatusLabels[value]}</button>)}<button className={!folderFilter ? "active" : ""} onClick={() => { setFolderFilter(null); setCategoryFilter(null); }} type="button">未整理</button></div>
      ...
      <AnimeFolderNavigation folders={data.folders} onChange={(folderId) => { setLibraryView("library"); setFolderFilter(folderId); setCategoryFilter(null); }} onFoldersChange={(folders) => setData((current) => ({ ...current, folders }))} scope="standard" selectedId={folderFilter} showUnorganized={false} />
      <div className="anime-bulk-toolbar"><button className={libraryView === "trash" ? "active secondary-button compact" : "secondary-button compact"} onClick={() => void openTrash("standard")} type="button">垃圾桶{trashData ? ` ${trashData.library.length}` : ""}</button>{libraryView === "trash" && <button className="secondary-button compact" onClick={() => setLibraryView("library")} type="button">返回我的動漫</button>}</div>
      {libraryView === "library" ? <><section className="anime-category-bar" aria-label="動漫類別">
        <div className="anime-category-scroll"><button className={!categoryFilter ? "active" : ""} onClick={() => setCategoryFilter(null)} type="button">所有類別</button>{data.tags.filter((category) => category.folderId === folderFilter).map((category) => <button className={categoryFilter === category.id ? "active" : ""} key={category.id} onClick={() => setCategoryFilter(category.id)} type="button">{category.name} <small>{data.library.filter((anime) => anime.tags.some((item) => item.id === category.id)).length}</small></button>)}<button aria-label="查看更多類別" className="anime-category-utility" onClick={() => setCategoryMoreOpen(true)} type="button">更多</button><button aria-label="新增類別" className="anime-category-utility anime-category-add-button" onClick={() => setCategoryAddOpen(true)} type="button">＋</button></div>
      </section>
      {!loaded ? <AnimeGridSkeleton /> : <AnimeCollectionList categories={data.tags} folders={data.folders} items={pagedLibrary} onMutated={async () => { await Promise.all([refresh(), refreshTrash("standard")]); }} onOpen={(anime) => { setSelectedReadOnly(false); setSelected(anime); }} scope="standard" />}
      ...
      {!library.length && <div className="anime-empty"><h3>{data.library.length ? "找不到符合的動漫" : "還沒有加入動漫"}</h3><button className="button compact" onClick={() => setAdding(true)} type="button">＋ 新增動漫</button></div>}</> : <AnimeCollectionList categories={trashData?.tags ?? data.tags} folders={data.folders} items={trashData?.library ?? []} onMutated={async () => { await Promise.all([refresh(), refreshTrash("standard")]); }} onOpen={(anime) => { setSelectedReadOnly(true); setSelected(anime); }} scope="standard" trashed />}
      ...
      <ModalDialog className="mobile-sheet-dialog" onClose={() => setCategoryMoreOpen(false)} open={categoryMoreOpen} title="動漫類別"><div className="anime-category-dialog"><p>選擇目前資料夾的類別以篩選我的動漫。</p><input aria-label="搜尋動漫類別" onChange={(event) => setCategoryQuery(event.target.value)} placeholder="搜尋類別" value={categoryQuery} /><div className="anime-category-manager-list"><button className={!categoryFilter ? "active" : ""} onClick={() => { setCategoryFilter(null); setCategoryMoreOpen(false); }} type="button">所有類別</button>{data.tags.filter((item) => item.folderId === folderFilter && item.name.toLocaleLowerCase().includes(categoryQuery.trim().toLocaleLowerCase())).map((item) => <button className={categoryFilter === item.id ? "active" : ""} key={item.id} onClick={() => { setCategoryFilter(item.id); setCategoryMoreOpen(false); }} type="button">{item.name} <small>{data.library.filter((anime) => anime.tags.some((tag) => tag.id === item.id)).length}</small></button>)}</div></div></ModalDialog>
    </>}
    {tab === "stats" && <AnimeStats data={data} />}
    {adding && <AnimeEditor adult={tab === "adult"} categories={tab === "adult" ? adultData?.tags ?? [] : data.tags} defaultFolderId={tab === "adult" ? adultFolderFilter : folderFilter} folders={tab === "adult" ? adultData?.folders ?? [] : data.folders} onClose={() => setAdding(false)} onSaved={async () => { setAdding(false); setPending("refresh"); try { if (tab === "adult") await refreshAdult(); else { await refresh(); setTab("library"); } setNotice(tab === "adult" ? "已新增成人作品。" : "已新增動漫。"); } finally { setPending(null); } }} />}
    {prefill && <AnimeEditor categories={data.tags} defaultFolderId={folderFilter} folders={data.folders} prefill={prefill} onClose={() => setPrefill(null)} onSaved={async () => { setPrefill(null); setPending("refresh"); try { await refresh(); setTab("library"); setNotice("已新增動漫。"); } finally { setPending(null); } }} />}
    {adultPrefill && <AnimeEditor adult categories={adultData?.tags ?? []} defaultFolderId={adultFolderFilter} folders={adultData?.folders ?? []} prefill={adultPrefill} onClose={() => setAdultPrefill(null)} onSaved={async () => { setAdultPrefill(null); await refreshAdult(); setNotice("已新增成人作品。"); }} />}
    {selected && <AnimeDetailDialog anime={selected} onClose={() => setSelected(null)} onEdit={selectedReadOnly ? undefined : () => { setEditing(selected); setSelected(null); }} />}
{editing && <AnimeEditor anime={editing} categories={editing.isAdult ? adultData?.tags ?? [] : data.tags} folders={editing.isAdult ? adultData?.folders ?? [] : data.folders} onClose={() => setEditing(null)} onRemove={() => { setRemoving(editing); setEditing(null); }} onSaved={async () => { const wasAdult = editing.isAdult; setPending("refresh"); try { if (wasAdult) { await refreshAdult(); setAdultView("library"); setTab("adult"); } else await refresh(); setEditing(null); setNotice("已儲存動漫資料。"); } finally { setPending(null); } }} />}
...
function AnimeGridSkeleton() {
  return <div aria-busy="true" aria-label="正在載入動漫" className="anime-grid anime-grid-skeleton">{Array.from({ length: 6 }, (_, index) => <div className="anime-card" key={index}><div className="anime-card-main"><span className="skeleton-block skeleton-cover" /><div className="anime-card-copy"><span className="skeleton-block skeleton-line short" /><span className="skeleton-block skeleton-line" /><span className="skeleton-block skeleton-line medium" /></div></div></div>)}</div>;
}
...
function AnimeEditor({ anime, prefill, adult = false, categories, folders, defaultFolderId = null, onClose, onSaved, onRemove }: { anime?: AnimeLibraryItem; prefill?: ExternalAnime; adult?: boolean; categories: AnimeTag[]; folders: AnimeWorkspaceData["folders"]; defaultFolderId?: string | null; onClose: () => void; onSaved: () => Promise<void>; onRemove?: () => void }) {
  ...
  const [folderId, setFolderId] = useState<string | null>(anime?.folderId ?? defaultFolderId);
  ...
  const save = async () => {
    ...
    try {
      ...
      const body = { ...(anime ? { id: anime.id } : {}), title, sourceUrl: sourceUrl.trim() || null, externalUrl: isAdult ? sourceUrl.trim() || null : undefined, isAdult, contentRating: isAdult ? contentRating.trim() || "成人內容" : null, adultSource: isAdult ? adultSource.trim() || "manual" : null, coverUrl: !cover && !anime ? prefill?.coverUrl ?? null : undefined, metadata: !anime && prefill ? prefill : undefined, coverTicket, watchStatus, rating, notes, folderId, categoryIds };
  ...
  return <ModalDialog onClose={onClose} open pending={pending} title={anime ? "修改動漫" : "新增動漫"}>
    <div className="anime-dialog">
      ...
      <label>資料夾<select onChange={(event) => { setFolderId(event.target.value || null); setCategoryIds([]); }} value={folderId ?? ""}><option value="">未整理</option>{folders.filter((folder) => folder.isVisible || folder.id === folderId).map((folder) => <option key={folder.id} value={folder.id}>{folder.name}</option>)}</select></label>
      <fieldset><legend>類別（可複選）</legend><div className="anime-tag-picker">{categories.filter((category) => category.folderId === folderId).length ? categories.filter((category) => category.folderId === folderId).map((category) => <label key={category.id}><input checked={categoryIds.includes(category.id)} onChange={() => setCategoryIds((ids) => ids.includes(category.id) ? ids.filter((id) => id !== category.id) : [...ids, category.id])} type="checkbox" /> {category.name}</label>) : <p className="anime-field-hint">先在這個資料夾的類別列按 ＋ 新增類別後即可選取。</p>}</div></fieldset>
