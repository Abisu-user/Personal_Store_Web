...
async function prepareImage(file: File) {
  ...
  try {
    ...
    return { dataUrl: canvas.toDataURL("image/webp", 0.88), lowQuality: image.naturalWidth < 1920 || image.naturalHeight < 1080 };
...
export function AppearanceSettings() {
  ...
  function reset() { setAppearance(appearanceDefaults); applyAppearance(appearanceDefaults); ["personal-vault:appearance:v1", "personal-vault:appearance:v2", "personal-vault:appearance:v3", "personal-vault:appearance:v4", "personal-vault:appearance:v5"].forEach((key) => window.localStorage.removeItem(key)); }
  async function chooseBackgrounds(files: FileList | null) {
    ...
    try {
      ...
      setImageNotice(images.some((item) => item.lowQuality) ? "已加入背景清單。原圖低於建議 1920 × 1080；放大或裁切後可能略為失真。" : `已加入 ${images.length} 張高畫質背景圖片。`);
