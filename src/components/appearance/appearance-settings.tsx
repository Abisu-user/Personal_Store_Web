...
import { Accent, Appearance, Background, BackgroundRotation, Density, Theme, activeBackground, appearanceDefaults, applyAppearance, normalizeHexColor, readAppearance, saveAppearance } from "@/lib/appearance/preferences";
...
async function prepareImage(file: File) {
  ...
  try {
    ...
    return { dataUrl: canvas.toDataURL("image/jpeg", 0.76), lowQuality: image.naturalWidth < 1600 || image.naturalHeight < 900 };
...
export function AppearanceSettings() {
  ...
  useEffect(() => {
    const stored = readAppearance(); applyAppearance(stored);
    const timer = window.setTimeout(() => { setAppearance(stored); setReady(true); }, 0);
  ...
  function reset() { setAppearance(appearanceDefaults); applyAppearance(appearanceDefaults); ["personal-vault:appearance:v1", "personal-vault:appearance:v2", "personal-vault:appearance:v3", "personal-vault:appearance:v4"].forEach((key) => window.localStorage.removeItem(key)); }
  async function chooseBackgrounds(files: FileList | null) {
    if (!files?.length) return;
    const selected = [...files];
    if (selected.some((file) => !["image/jpeg", "image/png", "image/webp"].includes(file.type) || file.size > 6_000_000)) { setImageNotice("請選擇單張 6 MB 以下的 JPG、PNG 或 WebP 圖片。"); return; }
    try {
      ...
      commit({ ...appearance, backgroundImages: merged, backgroundActiveIndex: activeIndex, backgroundImage: merged[activeIndex] });
      setImageNotice(images.some((item) => item.lowQuality) ? "已加入背景清單；其中有圖片低於建議 1600 × 900，放大裁切時可能失真。" : `已加入 ${images.length} 張背景圖片。`);
    } catch { setImageNotice("讀取圖片失敗，請改用 JPG、PNG 或 WebP。"); }
  }
  function selectImage(index: number) { commit({ ...appearance, backgroundActiveIndex: index, backgroundImage: appearance.backgroundImages[index], backgroundRotation: "manual" }); }
  ...
  const previewOverlay = appearance.backgroundOverlay ? "linear-gradient(rgb(255 255 255 / 42%), rgb(255 255 255 / 42%)), " : "";
  ...
  return <section aria-busy={!ready} className="appearance-workspace">
    ...
    <section className="appearance-section"><div><p className="eyebrow">BACKGROUND</p><h2>工作區背景</h2></div><div><div className="appearance-options background-options">{options.background.map(([value, label]) => <button aria-pressed={appearance.background === value} className={appearance.background === value ? "appearance-choice active" : "appearance-choice"} key={value} onClick={() => update("background", value as Background)} type="button"><i className={`background-swatch ${value}`} /><span>{label}</span></button>)}</div><div className="background-image-controls"><label>上傳圖片（最多 4 張）<input accept="image/jpeg,image/png,image/webp" multiple onChange={(event) => void chooseBackgrounds(event.target.files)} type="file" /></label>{appearance.backgroundImages.length > 0 && <><div aria-label="背景圖片清單" className="background-playlist">{appearance.backgroundImages.map((image, index) => <div className={appearance.backgroundActiveIndex === index ? "background-thumbnail active" : "background-thumbnail"} key={`${image.slice(-24)}-${index}`}><button aria-label={`選擇第 ${index + 1} 張背景`} onClick={() => selectImage(index)} style={{ backgroundImage: `url("${image}")` }} type="button" /><button aria-label={`移除第 ${index + 1} 張背景`} className="background-image-remove" onClick={() => removeImage(index)} type="button">×</button></div>)}</div><label>切換方式<select aria-label="背景切換方式" onChange={(event) => update("backgroundRotation", event.target.value as BackgroundRotation)} value={appearance.backgroundRotation}><option value="manual">手動選擇</option><option value="login">每次登入／重新開啟時</option><option value="interval">依時間自動切換</option></select></label>{appearance.backgroundRotation === "interval" && <label>切換間隔（分鐘）<input aria-label="背景切換間隔" inputMode="numeric" max="1440" min="1" onChange={(event) => update("backgroundRotationMinutes", Number(event.target.value))} type="number" value={appearance.backgroundRotationMinutes} /></label>}<div className="background-preview-wrap"><strong>工作區預覽</strong><div aria-label="背景預覽" className="background-preview" style={{ backgroundImage: `${previewOverlay}url("${previewImage}")`, backgroundPosition: appearance.backgroundOverlay ? `center, ${appearance.backgroundPositionX}% ${appearance.backgroundPositionY}%` : `${appearance.backgroundPositionX}% ${appearance.backgroundPositionY}%`, backgroundSize: appearance.backgroundOverlay ? `cover, ${appearance.backgroundZoom}%` : `${appearance.backgroundZoom}%`, filter: `brightness(${appearance.backgroundBrightness}%)` }}><span>預覽中的工作區</span></div></div><div className="crop-controls"><strong>自由裁切</strong><p>移動焦點或調整縮放，預覽與實際工作區會套用相同裁切。</p><label>水平焦點 <input aria-label="裁切水平焦點" max="100" min="0" onChange={(event) => update("backgroundPositionX", Number(event.target.value))} type="range" value={appearance.backgroundPositionX} /></label><label>垂直焦點 <input aria-label="裁切垂直焦點" max="100" min="0" onChange={(event) => update("backgroundPositionY", Number(event.target.value))} type="range" value={appearance.backgroundPositionY} /></label><label>放大 <input aria-label="裁切放大程度" max="180" min="100" onChange={(event) => update("backgroundZoom", Number(event.target.value))} type="range" value={appearance.backgroundZoom} /><output>{appearance.backgroundZoom}%</output></label><label>亮度 <input aria-label="背景亮度" max="150" min="60" onChange={(event) => update("backgroundBrightness", Number(event.target.value))} type="range" value={appearance.backgroundBrightness} /><output>{appearance.backgroundBrightness}%</output></label><label className="background-overlay-toggle"><input aria-label="啟用柔霧效果" checked={appearance.backgroundOverlay} onChange={(event) => update("backgroundOverlay", event.target.checked)} type="checkbox" /> 套用柔霧效果</label>{focusPositions.map(([value, label, x, y]) => <button className={appearance.backgroundPosition === value ? "active" : ""} key={value} onClick={() => commit({ ...appearance, backgroundPosition: value, backgroundPositionX: x, backgroundPositionY: y })} type="button">{label}</button>)}</div><label>背景調色<input aria-label="背景調色" onChange={(event) => update("backgroundTint", event.target.value)} type="color" value={appearance.backgroundTint ?? "#FFFFFF"} /></label></>} {imageNotice && <p className="background-image-notice">{imageNotice}</p>}</div></div></section>
