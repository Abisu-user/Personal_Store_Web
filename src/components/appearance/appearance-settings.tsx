...
function MobileNavigationPreview({ navigation }: { navigation: MobileNavigationPreferences }) {
  ...
  return <div className="mobile-navigation-live-preview" style={{ "--mobile-navigation-preview-count": navigation.sideCount * 2 + 1, "--mobile-nav-user-color": navigation.backgroundColor, "--mobile-nav-user-opacity": `${navigation.opacity}%` } as CSSProperties}>
    ...
    <button aria-label="更多，固定" className="mobile-navigation-preview-item fixed" disabled type="button"><AppIcon name="more" /><span>更多</span><small>固定</small></button>
  </div>;
}
export function AppearanceSettings() {
  ...
  const [navigationColorDraft, setNavigationColorDraft] = useState(mobileNavigationDefaults.backgroundColor);
  ...
  useEffect(() => {
    ...
    const restore = async () => {
      try {
        const hydrated = (await loadAccountAppearance({ all: true })).appearance;
        if (!active) return;
        applyAppearance(hydrated); setAppearance(hydrated); setNavigationColorDraft(hydrated.mobileNavigation.backgroundColor); setReady(true);
      } catch {
        if (active) { setAppearance(appearanceDefaults); setReady(true); }
      }
    };
    void restore();
    const onSyncError = () => setImageNotice("設定未能同步至帳號，請稍後再試。");
  ...
  function saveMobileNavigation(next: MobileNavigationPreferences) { update("mobileNavigation", normalizeMobileNavigationPreferences(next)); }
  function changeMobileNavigationCount(sideCount: MobileNavigationSideCount) { saveMobileNavigation({ ...mobileNavigation, sideCount }); }
  function changeMobileNavigationItem(key: MobileNavigationSlotKey, value: MobileNavigationDestination) {
    const slots = { ...mobileNavigation.slots };
    const previous = slots[key];
    const duplicate = (Object.entries(slots) as Array<[MobileNavigationSlotKey, MobileNavigationDestination]>).find(([slotKey, destination]) => slotKey !== key && destination === value);
  ...
  return <section aria-busy={!ready} className="appearance-workspace">
    <section className="appearance-preview"><div><p className="eyebrow">LIVE PREVIEW</p><h2>這是你的工作空間</h2><p>選擇會立即套用並同步至這個帳號的相同版面裝置。</p></div><div className="appearance-sample"><span>V</span><div><strong>今日焦點</strong><small>將常用資料整理得更舒服</small></div><i>◈</i></div></section>
    ...
    <section className="appearance-section"><div><p className="eyebrow">BOOKMARK VIEW</p><h2>網站收藏清單樣式</h2><p>手機與電腦會套用相同的顯示偏好。</p></div><div><div className="appearance-options">{options.bookmarkDisplay.map(([value, label]) => <button aria-pressed={appearance.bookmarkDisplay === value} className={appearance.bookmarkDisplay === value ? "appearance-choice active" : "appearance-choice"} key={value} onClick={() => update("bookmarkDisplay", value as BookmarkDisplay)} type="button"><i className={`bookmark-display-swatch ${value}`} /><span>{label}</span></button>)}</div>{appearance.bookmarkDisplay === "grid" && <label className="bookmark-grid-columns">每列格子數量 <input aria-label="每列格子數量" max="4" min="1" onChange={(event) => update("bookmarkGridColumns", Number(event.target.value))} type="range" value={appearance.bookmarkGridColumns} /><output>{appearance.bookmarkGridColumns} 格</output></label>}</div></section>
    <section className="appearance-section mobile-navigation-settings"><div><p className="eyebrow">MOBILE NAVIGATION</p><h2>手機底部導覽列</h2><p>首頁、新增與更多永久固定；其他位置、顏色與透明度會同步至同帳號的手機版。</p></div><div className="mobile-navigation-settings-body"><div className="mobile-navigation-layout-options">{([2, 3, 4] as MobileNavigationSideCount[]).map((sideCount) => <button aria-pressed={mobileNavigation.sideCount === sideCount} className={mobileNavigation.sideCount === sideCount ? "active" : ""} key={sideCount} onClick={() => changeMobileNavigationCount(sideCount)} type="button"><strong>左右各 {sideCount} 個</strong><span>共 {sideCount * 2 + 1} 格</span></button>)}</div><MobileNavigationPreview navigation={mobileNavigation} /><div className="mobile-navigation-slot-grid">{[...mobileNavigationVisibleSlotKeys(mobileNavigation.sideCount).left, ...mobileNavigationVisibleSlotKeys(mobileNavigation.sideCount).right].map((key, index) => <label key={key}>{index < mobileNavigation.sideCount - 1 ? `左側功能 ${index + 1}` : `右側功能 ${index - (mobileNavigation.sideCount - 2)}`}<select aria-label={`底部導覽${index < mobileNavigation.sideCount - 1 ? "左側" : "右側"}自訂功能`} id={`mobile-navigation-${key}`} onChange={(event) => changeMobileNavigationItem(key, event.target.value as MobileNavigationDestination)} value={mobileNavigation.slots[key]}>{mobileNavigationDestinations.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>)}</div><p className="mobile-navigation-settings-hint">若選到其他位置已使用的功能，兩個位置會自動交換；縮減格數不會刪除被隱藏的位置。</p><div className="mobile-navigation-visual-controls"><label>導覽列顏色<span><input aria-label="底部導覽列顏色" onChange={(event) => { setNavigationColorDraft(event.target.value.toUpperCase()); saveMobileNavigation({ ...mobileNavigation, backgroundColor: event.target.value }); }} type="color" value={mobileNavigation.backgroundColor} /><input aria-invalid={!/^#[0-9a-f]{3}(?:[0-9a-f]{3})?$/i.test(navigationColorDraft.trim())} aria-label="底部導覽列 HEX 色碼" onBlur={() => setNavigationColorDraft(mobileNavigation.backgroundColor)} onChange={(event) => changeMobileNavigationColor(event.target.value)} placeholder="#101117" spellCheck={false} value={navigationColorDraft} /></span></label><label>導覽列透明度 <output>{mobileNavigation.opacity}%</output><input aria-label="底部導覽列透明度" max="100" min="40" onChange={(event) => saveMobileNavigation({ ...mobileNavigation, opacity: Number(event.target.value) })} type="range" value={mobileNavigation.opacity} /></label></div><button className="secondary-button compact mobile-navigation-reset" onClick={resetMobileNavigation} type="button">恢復導覽列預設</button></div></section>
