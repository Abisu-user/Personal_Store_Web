...
export function CalendarWorkspace({ initialData }: { initialData: CalendarWorkspaceData }) {
  ...
  const selectEvent = (event: CalendarEvent) => { setEditorOpen(true); setSelectedId(event.id); setConfirmDelete(false); setDraft(toDraft(event)); setSelectedDay(dateKey(new Date(event.startsAt))); setNotice(null); };

  useEffect(() => {
    const open = () => { startNew(); setEditorOpen(true); };
