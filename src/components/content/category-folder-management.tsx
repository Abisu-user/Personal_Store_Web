...
export function CategoryFolderManagement({ bookmarks, code, files, initialTab = "bookmark", notes, photos }: { bookmarks: BookmarksWorkspaceData; notes: NotesWorkspaceData; code: CodeWorkspaceData; files: FilesWorkspaceData; initialTab?: Tab; photos: PhotosWorkspaceData }) {
  ...
  return <section className="management-workspace">
    ...
    <div className="management-sections"><details className="management-section" open><summary>類別 <span>管理分類與未分類資料</span></summary><ItemManager api="/api/taxonomy" contentKind={tab} count={categoryCount} description="類別只屬於目前選擇的資料類型；數字顯示使用這個類別的資料筆數。" fixedCount={active.filter((item) => !item.category).length} items={selected.categories} kind="category" onChange={selected.setCategories} title="類別" /></details><details className="management-section"><summary>資料夾 <span>管理資料夾與鎖定設定</span></summary><ItemManager api={selected.folderApi} count={folderCount} description="資料夾只屬於目前選擇的資料類型；數字不包含封存或垃圾桶中的資料。" items={selected.folders} kind={selected.folderKind} onChange={selected.setFolders} title={selected.folderTitle} /></details><details className="management-section"><summary>常駐清單 <span>我的最愛、置頂、封存與垃圾桶</span></summary><SmartFolderManager items={selected.items} tab={tab} /></details></div>
  </section>;
}
