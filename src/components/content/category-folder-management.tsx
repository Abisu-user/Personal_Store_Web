...
const tabs: Tab[] = ["bookmark", "note", "code", "file"];
...
export function CategoryFolderManagement({ bookmarks, code, files, notes }: { bookmarks: BookmarksWorkspaceData; notes: NotesWorkspaceData; code: CodeWorkspaceData; files: FilesWorkspaceData }) {
  ...
  const [bookmarkFolders, setBookmarkFolders] = useState<Item[]>(bookmarks.folders); const [noteFolders, setNoteFolders] = useState<Item[]>(notes.folders); const [codeFolders, setCodeFolders] = useState<Item[]>(code.folders); const [fileFolders, setFileFolders] = useState<Item[]>(files.folders);
  const selected = useMemo(() => tab === "bookmark" ? { items: bookmarks.bookmarks as ContentItem[], categories: bookmarkCategories, setCategories: setBookmarkCategories, folders: bookmarkFolders, setFolders: setBookmarkFolders, folderApi: "/api/taxonomy" as const, folderKind: "bookmark_folder", folderTitle: "收藏資料夾" } : tab === "note" ? { items: notes.notes as ContentItem[], categories: noteCategories, setCategories: setNoteCategories, folders: noteFolders, setFolders: setNoteFolders, folderApi: "/api/content-folders" as const, folderKind: "note", folderTitle: "筆記資料夾" } : tab === "code" ? { items: code.snippets as ContentItem[], categories: codeCategories, setCategories: setCodeCategories, folders: codeFolders, setFolders: setCodeFolders, folderApi: "/api/content-folders" as const, folderKind: "code", folderTitle: "程式碼資料夾" } : { items: files.files as ContentItem[], categories: fileCategories, setCategories: setFileCategories, folders: fileFolders, setFolders: setFileFolders, folderApi: "/api/content-folders" as const, folderKind: "file", folderTitle: "檔案資料夾" }, [bookmarkCategories, bookmarkFolders, bookmarks.bookmarks, code.snippets, codeCategories, codeFolders, fileCategories, fileFolders, files.files, noteCategories, noteFolders, notes.notes, tab]);
  const active = selected.items.filter((item) => !item.deletedAt);
  const categoryCount = (id: string) => active.filter((item) => item.category?.id === id).length;
  const folderCount = (id: string) => selected.items.filter((item) => item.folder?.id === id).length;
  return <section className="management-workspace"><div className="management-tabs" role="tablist">{tabs.map((key) => <button aria-selected={tab === key} className={tab === key ? "active" : ""} key={key} onClick={() => setTab(key)} role="tab" type="button">{tabLabels[key]}</button>)}</div><div className="management-intro"><p className="eyebrow">{tabLabels[tab].toUpperCase()} ORGANIZATION</p><h2>管理 {tabLabels[tab]} 的類別與資料夾</h2><p>「未分類」固定保留；每種資料各自有獨立類別。資料夾、常駐清單與數量會立即反映目前設定。</p></div><div className="management-grid"><ItemManager api="/api/taxonomy" contentKind={tab} count={categoryCount} description="類別只屬於目前選擇的資料類型；數字顯示使用這個類別的資料筆數。" fixedCount={active.filter((item) => !item.category).length} items={selected.categories} kind="category" onChange={selected.setCategories} title="類別" /><ItemManager api={selected.folderApi} count={folderCount} description="資料夾只屬於目前選擇的資料類型；數字包含資料夾內所有資料。" items={selected.folders} kind={selected.folderKind} onChange={selected.setFolders} title={selected.folderTitle} /></div><SmartFolderManager items={selected.items} tab={tab} /></section>;
}
