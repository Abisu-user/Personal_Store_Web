...
const moreItems = [["/notes", "□", "筆記"], ["/code", "⌘", "程式碼"], ["/photos", "▧", "照片"], ["/vocabulary", "文", "單字學習"], ["/vault", "◈", "保管庫"], ["/calendar", "◌", "日曆"], ["/organize", "☷", "管理類別／資料夾"], ["/appearance", "◐", "外觀與布局"], ["/security", "⌁", "安全中心"], ["/security/mfa", "◈", "雙因素驗證"], ["/profile", "●", "帳號設定"]] as const;
export function MobileAppNavigation() {
  const pathname = usePathname(); const router = useRouter(); const [moreOpen, setMoreOpen] = useState(false); const [signingOut, setSigningOut] = useState(false); const [pendingPath, setPendingPath] = useState<string | null>(null);
