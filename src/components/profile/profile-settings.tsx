...
export function ProfileSettings({ initialProfile, email }: ProfileSettingsProps) {
  ...
  function saveProfile(event: FormEvent<HTMLFormElement>) {
    ...
    backgroundJobs.enqueue({
      ...
      operation: "更新設定",
      page: "/profile",
      entityKey: "profile:current-user",
      mergeKey: "profile:current-user",
      request: { url: "/api/profile", method: "PATCH", body: { action: "profile", ...profile } },
      persist: false,
      onSuccess: (value) => { const result = value as { profile?: { username: string; display_name?: string | null; avatar_path: ProfileAvatar }; message?: string } | null; if (result?.profile) setProfile((current) => ({ ...current, username: result.profile!.username, displayName: result.profile!.display_name ?? "", avatar: result.profile!.avatar_path })); setMessage(result?.message ?? "個人資料已儲存。"); window.dispatchEvent(new CustomEvent("personal-vault:profile-updated", { detail: result?.profile })); },
      onError: (cause) => setError(cause.message || "無法儲存個人資料。"),
    });
  }
  ...
  return <div className="profile-workspace">
    {passwordPending && <OperationStatus label="正在更新密碼…" />}
    ...
    <form className="profile-panel" onSubmit={saveProfile}>
      ...
      <button className="button" type="submit">儲存個人資料</button>
    </form>
