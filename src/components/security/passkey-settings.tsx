...
export function PasskeySettings() {
  ...
  async function disableAutoLock(pin: string) {
    setMessage(null);
    ...
    if (!response.ok) throw await pinVerificationErrorFromResponse(response, "PIN 驗證失敗，未變更自動鎖定設定。");
  }

  function finishDisablingAutoLock() {
    setAutoLockEnabled(false);
    setDisableOpen(false);
    window.dispatchEvent(new Event(pinStatusEvent));
    setMessage("App 自動鎖定已關閉。");
  }
  ...
  return <section className="passkey-settings">
    ...
    <ModalDialog onClose={() => { if (!disableBusy) setDisableOpen(false); }} open={disableOpen} pending={disableBusy} title="關閉 App 自動鎖定">{pinConfigured ? <GlassyPinVerification
      ...
      title="再次驗證 App PIN"
      description="驗證成功後才會關閉 App 自動鎖定。"
