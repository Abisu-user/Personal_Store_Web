...
type CipherItem = { id: string; ciphertext: string; nonce: string; aad: { entryId: string; version: 1 }; categoryId: string | null; categoryIds?: string[] };
type PlainItem = { id: string; label: string; username: string; secret: string; notes: string; categoryId: string | null; categoryIds: string[] };
...
export function VaultWorkspace() {
  ...
  return <section className={`vault-workspace ${mobileStyles.vaultWorkspace}`}>
    ...
    <section className="vault-categories" aria-label="保管項目分類" data-chip-overflow-container><div className="vault-category-heading"><strong>分類</strong><div data-chip-overflow-actions><button className="secondary-button compact" onClick={openManager} type="button">管理</button><button className="button compact" onClick={() => { setError(null); setCategoryAddOpen(true); }} type="button">＋ 新增</button></div></div><div className="vault-category-row"><ResponsiveChipOverflow activeIds={categoryFilters.filter((id) => id !== "unclassified")} className="vault-filter-chips" items={categories} itemId={(category) => category.id} itemMeasureKey={(category) => `${category.name}|${category.itemCount}`} leading={<><button className={!categoryFilters.length ? "active" : ""} onClick={() => selectCategoryFilter("all")} type="button">全部</button><button className={categoryFilters.includes("unclassified") ? "active" : ""} onClick={() => selectCategoryFilter("unclassified")} type="button">未分類</button></>} leadingCount={2} renderItem={(category) => <button className={categoryFilters.includes(category.id) ? "active" : ""} key={category.id} onClick={() => selectCategoryFilter(category.id)} type="button">{category.name}{category.itemCount ? ` ${category.itemCount}` : ""}</button>} renderMore={(hasHiddenActive) => <button aria-expanded={categoryMoreOpen} className={hasHiddenActive ? "active" : ""} onClick={() => setCategoryMoreOpen((current) => !current)} type="button">更多</button>} rowClassName="vault-filter-chips-row" />{categoryMoreOpen && <div className="vault-more-categories">{categories.map((category) => <button className={categoryFilters.includes(category.id) ? "active" : ""} key={category.id} onClick={() => selectCategoryFilter(category.id)} type="button">{category.name}{category.itemCount ? ` ${category.itemCount}` : ""}</button>)}</div>}</div></section>
    {selectionMode && <section className="vault-batch-toolbar"><label><input checked={filteredItems.length > 0 && filteredItems.every((item) => selectedItemIds.has(item.id))} onChange={(event) => setSelectedItemIds(event.target.checked ? new Set(filteredItems.map((item) => item.id)) : new Set())} type="checkbox" />全選目前清單</label></section>}
    <BatchActionBar count={selectedItemIds.size} onCancel={closeSelectionMode}><button className="secondary-button" disabled={pending} onClick={() => setBatchMoveOpen(true)} type="button">整理</button><button className="delete-button" disabled={pending} onClick={() => setBatchDeleteOpen(true)} type="button">刪除</button></BatchActionBar>
    <div className="vault-grid">{filteredItems.map((item) => { const visible = visibleItemIds.has(item.id); const selected = selectedItemIds.has(item.id); return <article className={selected ? "vault-item selected" : "vault-item"} key={item.id}><header>{selectionMode ? <label className="vault-item-select"><input aria-label={`選取 ${item.label}`} checked={selected} onChange={() => toggleItemSelection(item.id)} type="checkbox" /><span>選取</span></label> : <span className="vault-kind-badge"><i aria-hidden="true">⌑</i>{categoryLabel(item.categoryIds)}</span>}<div className="vault-item-menu"><button aria-expanded={menuItemId === item.id} aria-label={`開啟 ${item.label} 的操作選單`} className="vault-menu-trigger" onClick={() => setMenuItemId((current) => current === item.id ? null : item.id)} type="button">⋯</button>{menuItemId === item.id && <div className="vault-item-menu-popover"><button onClick={() => { setError(null); setEditing(item); setItemDialogOpen(true); }} type="button">編輯</button><button className="vault-menu-delete" disabled={pending} onClick={() => setDeleting(item)} type="button">刪除</button></div>}</div></header><div className="vault-item-content"><h3>{item.label}</h3>{item.username && <p className="vault-username">{item.username}</p>}<code aria-label={visible ? "敏感內容已顯示" : "敏感內容已遮蔽"}>{visible ? item.secret : maskedSecret(item.secret)}</code>{item.notes && <p className="vault-notes">{item.notes}</p>}</div><footer><button className="secondary-button compact" onClick={() => setVisibleItemIds((current) => { const next = new Set(current); if (next.has(item.id)) next.delete(item.id); else next.add(item.id); return next; })} type="button">{visible ? "隱藏" : "顯示"}</button><button className="secondary-button compact" onClick={() => void copySecret(item)} type="button">複製</button></footer></article>; })}</div>
    ...
    <CreateItemModal className="vault-item-dialog" onClose={closeItemDialog} open={itemDialogOpen} pending={pending} title={editing ? "編輯保管項目" : "新增保管項目"}><form className="vault-item-form" key={editing?.id ?? "new"} onSubmit={saveItem}>{error && <p className="notice error" role="alert">{error}</p>}<label>項目名稱<input defaultValue={editing?.label ?? ""} name="label" placeholder="例如 GitHub、主要銀行" required /></label><TaxonomyMultiSelect categories={categories.map((category) => ({ id: category.id, name: category.name, folder_id: null }))} defaultCategoryIds={editing?.categoryIds ?? []} folders={[]} showFolders={false} /><label>帳號／使用者名稱<input defaultValue={editing?.username ?? ""} name="username" placeholder="選填" /></label><label>密碼或敏感內容<textarea defaultValue={editing?.secret ?? ""} name="secret" placeholder="密碼、API Key、Recovery Code…" required rows={4} /></label><label>備註<textarea defaultValue={editing?.notes ?? ""} name="notes" placeholder="選填" rows={3} /></label><CreateFormActions pending={pending} label={editing ? "重新加密並儲存" : "加密儲存"} pendingLabel="加密儲存中…" onCancel={closeItemDialog} /></form></CreateItemModal>
