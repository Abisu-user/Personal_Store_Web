...
export function FlashcardReview({ cards, onReview, pending }: { cards: VocabularyCard[]; onReview: (card: VocabularyCard, rating: ReviewRating) => Promise<void>; pending: boolean }) {
  const [index, setIndex] = useState(0); const [flipped, setFlipped] = useState(false); const [exampleMode, setExampleMode] = useState(false);
  ...
  return <section className="flashcard-session"><div className="flashcard-progress"><span>今日複習</span><strong>{Math.min(index + 1, cards.length)} / {cards.length}</strong></div><button className="secondary-button compact" disabled={!example} onClick={() => { setExampleMode((value) => !value); setFlipped(false); }} type="button">{exampleMode ? "一般模式" : "例句模式"}</button><button aria-label="翻轉單字卡" className={`flashcard${flipped ? " flipped" : ""}`} onClick={() => setFlipped((value) => !value)} type="button"><span className="flashcard-front"><small>{exampleMode ? "從例句猜單字" : card.language.toUpperCase()}</small><strong>{exampleMode && example ? example.sentence.replace(card.word, "＿＿＿") : card.word}</strong><em>{exampleMode ? "點擊顯示答案" : card.reading || card.romaji || "點擊顯示答案"}</em></span><span className="flashcard-back"><small>{exampleMode ? example?.translationZhTw || example?.translation || "例句翻譯" : card.partOfSpeech || "Vocabulary"}</small><strong>{exampleMode ? card.word : card.primaryTranslation || card.meanings.map((meaning) => meaning.meaning).join("、") || "尚未填寫意思"}</strong><em>{exampleMode ? card.primaryTranslation || "尚未填寫意思" : example?.sentence || card.notes || "沒有補充內容"}</em></span></button>{flipped ? <div className="review-ratings">{labels.map((item) => <button className={item.className} disabled={pending} key={item.rating} onClick={() => void rate(item.rating)} type="button">{item.label}</button>)}</div> : <button className="button flashcard-reveal" onClick={() => setFlipped(true)} type="button">顯示答案</button>}</section>;
}
