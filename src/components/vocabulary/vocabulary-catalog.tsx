...
export function VocabularyCatalog({ onChanged }: { onChanged: () => Promise<void> }) {
  ...
  useEffect(() => { setPage(1); setStartsWith(""); setChosen(new Set()); }, [language, submittedQuery]);
  ...
  return <section className="vocabulary-catalog" aria-busy={loading}>
    ...
    <div className="vocabulary-catalog-filters"><div className="vocabulary-filter-chips"><button className={!level ? "active" : ""} onClick={() => { setLevel(""); setPage(1); setStartsWith(""); }} type="button">全部</button>{language === "ja" ? ["N5", "N4", "N3", "N2", "N1"].map((item) => <button className={level === item ? "active" : ""} key={item} onClick={() => { setLevel(item); setPage(1); setStartsWith(""); }} type="button">{item}</button>) : <span>職場、商務與 TOEIC 常見單字</span>}</div><div className="vocabulary-catalog-index" aria-label="字首索引">{(language === "ja" ? kana : letters).map((item) => <button className={startsWith === item ? "active" : ""} key={item} onClick={() => { setStartsWith(startsWith === item ? "" : item); setPage(1); }} type="button">{item}</button>)}</div></div>
