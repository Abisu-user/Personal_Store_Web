...
function CatalogExamplesDialog({ item, onClose, onChanged }: { item: CatalogItem | null; onClose: () => void; onChanged: () => Promise<void> }) {
  ...
  useEffect(() => { if (item) { setData(null); setShowReadings(false); setAiPreview([]); setSentence(""); setReading(""); setTranslation(""); void load(); } }, [item, load]);
