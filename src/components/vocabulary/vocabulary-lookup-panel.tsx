...
export function VocabularyLookupPanel({ cards, mode = "lookup", onAdd }: { cards: VocabularyCard[]; mode?: "lookup" | "assistant"; onAdd: (card: PreparedCard) => void }) {
  ...
  async function lookup(event?: FormEvent, nextQuery = query) {
    ...
    if (!value) return;
    setLoading(true); setError(null); setItems([]); setSubmittedQuery(value);
    try {
  ...
  return <><style>{`${lookupCss}${lookupSurfaceCss}`}</style><section className="vocabulary-lookup">
    <div className="vocabulary-lookup-intro"><div><p className="eyebrow">{mode === "lookup" ? "ONLINE DICTIONARY" : "AI LANGUAGE ASSISTANT"}</p><h2>{mode === "lookup" ? "搜尋單字" : "AI 單字學習"}</h2><p>{mode === "lookup" ? "先選擇日文或英文；輸入中文、日文或英文後，按搜尋才會查詢。" : "詢問用法、單字差異、常用搭配或翻譯；AI 建議不會自動寫入單字庫。"}</p></div><div aria-label="選擇字典語言" className="vocabulary-language-switch"><button className={language === "ja" ? "active" : ""} onClick={() => { setLanguage("ja"); setItems([]); setSubmittedQuery(""); }} type="button">🇯🇵 日文</button><button className={language === "en" ? "active" : ""} onClick={() => { setLanguage("en"); setItems([]); setSubmittedQuery(""); }} type="button">🇬🇧 English</button></div></div>
    {mode === "lookup" && <form className="vocabulary-lookup-form" onSubmit={(event) => void lookup(event)}><input aria-label="搜尋中文、日文或英文單字" minLength={1} onChange={(event) => { setQuery(event.target.value); setItems([]); setSubmittedQuery(""); setError(null); }} placeholder={language === "ja" ? "輸入中文、日文或英文，查日文字典" : "輸入中文、日文或英文，查英文字典"} value={query} /><button className="button" disabled={loading || !query.trim()} type="submit">{loading ? "查詢中…" : "搜尋"}</button></form>}
