...
const lookupSurfaceCss = `.vocabulary-lookup-intro,.vocabulary-history,.vocabulary-lookup-results section,.vocabulary-ai-panel{border:1px solid color-mix(in srgb,var(--line) 86%,transparent);border-radius:18px;background:color-mix(in srgb,var(--surface) 94%,var(--canvas));box-shadow:0 10px 28px rgb(20 42 80 / 6%);backdrop-filter:blur(12px) saturate(1.04);-webkit-backdrop-filter:blur(12px) saturate(1.04)}.vocabulary-lookup-intro,.vocabulary-history{padding:16px}.vocabulary-history>div{margin-top:2px}.vocabulary-language-switch button,.vocabulary-history button{background:color-mix(in srgb,var(--surface) 96%,var(--canvas))}.vocabulary-translation-list{display:grid;gap:5px;margin:2px 0}.vocabulary-translation-list p{display:grid;grid-template-columns:78px minmax(0,1fr);gap:8px;margin:0}.vocabulary-translation-list b{color:var(--muted);font-size:.78rem}.vocabulary-translation-list span{overflow-wrap:anywhere}.vocabulary-lookup-card{cursor:pointer}.vocabulary-lookup-card:focus-visible{outline:3px solid color-mix(in srgb,var(--brand) 38%,transparent);outline-offset:2px}.vocabulary-entry-detail{display:grid;gap:16px}.vocabulary-entry-detail section{display:grid;gap:8px}.vocabulary-entry-detail h3{margin:0;font-size:1rem}.vocabulary-entry-detail .vocabulary-translation-list{padding:13px;border:1px solid var(--line);border-radius:12px;background:color-mix(in srgb,var(--surface) 92%,var(--canvas))}.vocabulary-entry-detail ul{margin:0;padding-left:20px}.vocabulary-entry-detail p{margin:0;color:var(--muted);line-height:1.6}@media(max-width:700px){.vocabulary-lookup{gap:16px}.vocabulary-lookup-intro,.vocabulary-history,.vocabulary-lookup-results section,.vocabulary-ai-panel{padding:14px;border-radius:15px}.vocabulary-lookup-intro h2{font-size:1.35rem}.vocabulary-lookup-intro p{font-size:.88rem;line-height:1.55}.vocabulary-language-switch button{min-height:38px;padding-inline:10px;font-size:.86rem}.vocabulary-lookup-form{gap:8px}.vocabulary-lookup-form .button{min-height:42px}.vocabulary-lookup-card{min-height:0;padding:14px}.vocabulary-ai-actions textarea{min-height:108px}}`;

export function VocabularyLookupPanel({ cards, mode = "lookup", onAdd }: { cards: VocabularyCard[]; mode?: "lookup" | "assistant"; onAdd: (card: PreparedCard) => void }) {
  const [language, setLanguage] = useState<Language>("ja");
  const [query, setQuery] = useState("");
  const [submittedQuery, setSubmittedQuery] = useState("");
  const [items, setItems] = useState<LookupEntry[]>([]);
  const [relatedItems, setRelatedItems] = useState<LookupEntry[]>([]);
  const [detailEntry, setDetailEntry] = useState<LookupEntry | null>(null);
  ...
  async function lookup(event?: FormEvent, nextQuery = query) {
    ...
    try {
      const response = await fetch(`/api/vocabulary/lookup?language=${language}&q=${encodeURIComponent(value)}`, { cache: "no-store", signal: controller.signal });
      const body = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(body.error || "字典查詢失敗。");
      setItems(body.exact || body.items || []);
      setRelatedItems(body.related || []);
      setSubmittedQuery(value);
      await loadHistory();
    } catch (cause) {
      if (cause instanceof DOMException && cause.name === "AbortError") return;
      setError(cause instanceof Error ? cause.message : "字典查詢失敗。");
    } finally {
      if (lookupAbort.current === controller) setLoading(false);
    }
  }
  ...
  function displayRows(entry: LookupEntry) {
    ...
    return entry.language === "ja"
      ? [["日文漢字", entry.translations?.japanese || entry.word], ["讀音", entry.reading || entry.kana || "讀音未提供"], ["片假名", katakana || "讀音未提供"], ["中文意思", entry.translations?.chinese || "暫無可用中文翻譯（翻譯服務未回應）"]]
  ...
  return <><style>{`${lookupCss}${lookupSurfaceCss}`}</style><section className="vocabulary-lookup">
    <div className="vocabulary-lookup-intro"><div><p className="eyebrow">{mode === "lookup" ? "ONLINE DICTIONARY" : "AI LANGUAGE ASSISTANT"}</p><h2>{mode === "lookup" ? "搜尋單字" : "AI 單字學習"}</h2><p>{mode === "lookup" ? "字典先提供可驗證的詞義；AI 僅在你主動詢問時提供學習說明。" : "詢問用法、單字差異、常用搭配或翻譯；AI 建議不會自動寫入單字庫。"}</p></div><div aria-label="選擇字典語言" className="vocabulary-language-switch"><button className={language === "ja" ? "active" : ""} onClick={() => { setLanguage("ja"); setItems([]); setRelatedItems([]); setSubmittedQuery(""); }} type="button">🇯🇵 日文</button><button className={language === "en" ? "active" : ""} onClick={() => { setLanguage("en"); setItems([]); setRelatedItems([]); setSubmittedQuery(""); }} type="button">🇬🇧 English</button></div></div>
    {mode === "lookup" && <form className="vocabulary-lookup-form" onSubmit={(event) => void lookup(event)}><input aria-label="搜尋中文、日文或英文單字" minLength={1} onChange={(event) => { setQuery(event.target.value); setError(null); }} placeholder={language === "ja" ? "輸入中文、日文或英文，查日文字典" : "輸入中文、日文或英文，查英文字典"} value={query} /><button className="button" disabled={loading || !query.trim()} type="submit">{loading ? "查詢中…" : "搜尋"}</button></form>}
    {mode === "lookup" && history.length > 0 && <div className="vocabulary-history"><strong>最近搜尋</strong><div>{history.map((item) => <button key={item} onClick={() => { setQuery(item); void lookup(undefined, item); }} type="button">{item}</button>)}<button className="text-button" onClick={async () => { await fetch(`/api/vocabulary/lookup?language=${language}`, { method: "DELETE" }); setHistory([]); }} type="button">清除</button></div></div>}
    {error && <p className="notice error" role="alert">{error}</p>}
    {mode === "lookup" && submittedQuery && <div className="vocabulary-lookup-results">
      <section><p className="eyebrow">ONLINE DICTIONARY</p><h3>主要字典結果</h3>{loading ? <p className="vocabulary-result-empty">正在查詢可靠字典資料…</p> : (items.length || relatedItems.length) ? <><p className="vocabulary-result-empty">{items.length ? "先顯示精確詞、讀音與可能的辭書形；延伸詞放在後方。" : "沒有精確詞條，以下是可能的延伸結果。"}</p><div className="vocabulary-lookup-grid">{[...items, ...relatedItems].map((entry) => { const exists = cards.some((card) => card.language === entry.language && card.word.trim().toLocaleLowerCase() === entry.word.trim().toLocaleLowerCase()); const rows = displayRows(entry); const matchLabel = entry.matchKind === "exact" ? "精確結果" : entry.matchKind === "dictionary-form" ? `辭書形：${entry.dictionaryForm}` : "其他可能結果"; return <article aria-label={`查看 ${entry.word} 的詳細資訊`} className="vocabulary-lookup-card" key={entry.id} onClick={() => setDetailEntry(entry)} onKeyDown={(event) => { if (event.target === event.currentTarget && (event.key === "Enter" || event.key === " ")) { event.preventDefault(); setDetailEntry(entry); } }} role="button" tabIndex={0}><small>{matchLabel} · {entry.source} · {entry.partOfSpeech || "詞性未提供"}</small><div className="vocabulary-translation-list">{rows.map(([label, value]) => <p key={label}><b>{label}</b><span>{value}</span></p>)}</div><div className="vocabulary-result-actions" onClick={(event) => event.stopPropagation()}><button className="secondary-button compact" onClick={() => { setAssistantPrompt(`${entry.word}${entry.reading ? `（${entry.reading}）` : ""} 怎麼用？請用繁體中文說明語感並給四個情境例句。`); setAssistantAction("explain"); void askAi(`${entry.word}${entry.reading ? `（${entry.reading}）` : ""} 怎麼用？請用繁體中文說明語感並給四個情境例句。`, "explain"); }} type="button">✨ AI 詳細解釋</button>{exists ? <span className="vocabulary-added">✓ 已在單字庫</span> : <button className="button compact" onClick={() => prepare(entry)} type="button">＋ 加入單字庫</button>}</div></article>; })}</div></> : <p className="vocabulary-result-empty">找不到符合的字典資料。請改用更完整的單字或詞語再試一次。</p>}</section>
    </div>}
    <ModalDialog onClose={() => setDetailEntry(null)} open={Boolean(detailEntry)} title={detailEntry?.word ?? "單字詳細資訊"}>{detailEntry && <div className="vocabulary-entry-detail"><div className="vocabulary-translation-list">{displayRows(detailEntry).map(([label, value]) => <p key={label}><b>{label}</b><span>{value}</span></p>)}</div><section><h3>字典定義</h3>{detailEntry.meanings.length ? <ul>{detailEntry.meanings.map((meaning) => <li key={meaning}>{meaning}</li>)}</ul> : <p>字典未提供定義。</p>}</section>{detailEntry.examples.length > 0 && <section><h3>例句</h3>{detailEntry.examples.map((example, index) => <p key={`${example.sentence}-${index}`}><b>{example.sentence}</b>{example.translation ? <><br />{example.translation}</> : null}</p>)}</section>}{detailEntry.synonyms.length > 0 && <section><h3>同義詞</h3><p>{detailEntry.synonyms.join("、")}</p></section>}{detailEntry.antonyms.length > 0 && <section><h3>反義詞</h3><p>{detailEntry.antonyms.join("、")}</p></section>}<div className="dialog-actions"><button className="secondary-button" onClick={() => setDetailEntry(null)} type="button">關閉</button><button className="button" onClick={() => { prepare(detailEntry); setDetailEntry(null); }} type="button">＋ 加入單字庫</button></div></div>}</ModalDialog>
