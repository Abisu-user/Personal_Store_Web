...
export function VocabularyLookupPanel({ cards, onAdd, onOpenCard }: { cards: VocabularyCard[]; onAdd: (card: PreparedCard) => void; onOpenCard: (card: VocabularyCard) => void }) {
  ...
  const localMatches = useMemo(() => query.trim().length < 1 ? [] : cards.filter((card) => card.language === language && cardText(card).includes(query.trim().toLocaleLowerCase())).slice(0, 6), [cards, language, query]);
  ...
  return <><style>{`${lookupCss}${lookupSurfaceCss}`}</style><section className="vocabulary-lookup">
    ...
    <form className="vocabulary-lookup-form" onSubmit={(event) => void lookup(event)}><input aria-label={language === "ja" ? "搜尋日文單字" : "搜尋英文單字"} minLength={1} onChange={(event) => setQuery(event.target.value)} placeholder={language === "ja" ? "搜尋日文、讀音或中文意思" : "Search English / 中文意思"} value={query} /><button className="button" disabled={loading || !query.trim()} type="submit">{loading ? "查詢中…" : "搜尋"}</button></form>
    ...
    {query.trim() && <div className="vocabulary-lookup-results">
      <section><p className="eyebrow">MY VOCABULARY</p><h3>我的單字</h3>{localMatches.length ? <div className="vocabulary-lookup-grid">{localMatches.map((card) => <button className="vocabulary-lookup-card local" key={card.id} onClick={() => onOpenCard(card)} type="button"><small>{card.language === "ja" ? "日文" : "English"} · 已加入</small><strong>{card.word}</strong><span>{card.reading || card.romaji || card.primaryTranslation || "查看詳細資料"}</span></button>)}</div> : <p className="vocabulary-result-empty">你的{language === "ja" ? "日文" : "英文"}單字庫中尚未找到符合資料。</p>}</section>
      <section><p className="eyebrow">ONLINE DICTIONARY</p><h3>線上字典結果</h3>{loading ? <p className="vocabulary-result-empty">正在查詢可靠字典資料…</p> : items.length ? <div className="vocabulary-lookup-grid">{items.map((entry) => { const exists = cards.some((card) => card.language === entry.language && card.word.trim().toLocaleLowerCase() === entry.word.trim().toLocaleLowerCase()); return <article className="vocabulary-lookup-card" key={entry.id}><small>{entry.source} · {entry.partOfSpeech || "詞性未提供"}</small><h4>{entry.word}</h4>{entry.reading && <p>{entry.reading}</p>}{entry.ipa && <p>{entry.ipa}</p>}<p>{entry.primaryTranslation || entry.meanings.slice(0, 3).join("；") || "字典未提供定義"}</p><div className="vocabulary-result-actions"><button className="secondary-button compact" onClick={() => { setAssistantPrompt(`${entry.word}${entry.reading ? `（${entry.reading}）` : ""} 怎麼用？請用繁體中文說明語感並給四個情境例句。`); setAssistantAction("explain"); void askAi(`${entry.word}${entry.reading ? `（${entry.reading}）` : ""} 怎麼用？請用繁體中文說明語感並給四個情境例句。`, "explain"); }} type="button">✨ 問 AI</button>{exists ? <span className="vocabulary-added">✓ 已在單字庫</span> : <button className="button compact" onClick={() => prepare(entry)} type="button">＋ 加入單字庫</button>}</div></article>; })}</div> : <p className="vocabulary-result-empty">{query && !loading ? "找不到符合的字典資料。若是中文反查或特殊詞彙，可直接問 AI 協助判斷。" : "輸入後開始查詢。"}</p>}</section>
    </div>}
