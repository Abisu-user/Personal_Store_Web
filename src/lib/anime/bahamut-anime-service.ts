...
export async function findBahamutAnime(titles: Array<string | null | undefined>, callerSignal?: AbortSignal): Promise<BahamutAnimeMatch> {
  ...
  const response = await fetch(url, { headers: { Accept: "text/html,application/xhtml+xml", "User-Agent": "Personal-Vault-Anime-Catalogue/1.0" }, cache: "no-store", signal });
  if (!response.ok) throw new Error(`Bahamut catalogue returned ${response.status}`);
  const match = findCandidates(await response.text(), names)[0];
