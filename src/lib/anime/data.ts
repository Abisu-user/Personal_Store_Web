...
export async function getAnimeWorkspaceData(userId: string, scope: "standard" | "adult" = "standard", options: { trashed?: boolean } = {}): Promise<AnimeWorkspaceData> {
  ...
  const folderIdsByAnime = new Map<string, string[]>();
  if (!folderLinkError) (folderLinks ?? []).forEach((link: any) => { const current = folderIdsByAnime.get(link.anime_id) ?? []; current.push(link.folder_id); folderIdsByAnime.set(link.anime_id, current); });
