...
export async function getAnimeWorkspaceData(userId: string): Promise<AnimeWorkspaceData> {
  ...
  const [{ data: rows, error: libraryError }, { data: tagRows, error: tagError }, { data: logRows, error: logError }, { data: preferenceRow, error: preferenceError }] = await Promise.all([
    ...
    admin.from("anime_preferences").select("preferred_streaming_platform").eq("user_id", userId).maybeSingle(),
  ]);
  if (libraryError || tagError || logError || preferenceError) throw new Error("Anime library unavailable");
  ...
  const preferredStreamingPlatform = preferenceRow?.preferred_streaming_platform;
  return { library, tags, logs, preferences: { preferredStreamingPlatform: preferredStreamingPlatform === "netflix" || preferredStreamingPlatform === "crunchyroll" || preferredStreamingPlatform === "other" ? preferredStreamingPlatform : "bahamut" } };
}
