...
const anilistQuery = `query AnimeLibrary($search: String, $id: Int) { Page(page: 1, perPage: 12) { media(search: $search, id: $id, type: ANIME, isAdult: false, sort: [POPULARITY_DESC]) { id idMal title { romaji english native userPreferred } synonyms popularity coverImage { extraLarge large medium } bannerImage description(asHtml: false) format status episodes duration season seasonYear startDate { year month day } endDate { year month day } averageScore genres studios { nodes { name } } source isAdult siteUrl relations { edges { relationType node { id type title { romaji english native } } } } } } }`;
const anilistSearchPageQuery = `query AnimeSearchPage($search: String!, $page: Int!, $perPage: Int!) { Page(page: $page, perPage: $perPage) { pageInfo { currentPage hasNextPage total } media(search: $search, type: ANIME, isAdult: false, sort: [SEARCH_MATCH, POPULARITY_DESC]) { id idMal title { romaji english native userPreferred } synonyms popularity coverImage { extraLarge large medium } bannerImage description(asHtml: false) format status episodes duration season seasonYear startDate { year month day } endDate { year month day } averageScore genres studios { nodes { name } } source isAdult siteUrl relations { edges { relationType node { id type title { romaji english native } } } } } } }`;
...
function mapAniList(row: any): ExternalAnime {
  ...
  return { id: String(row?.id ?? ""), source: "anilist", title: text(row?.title?.romaji) ?? text(row?.title?.english) ?? text(row?.title?.native) ?? "未命名動漫", titleJapanese: text(row?.title?.native), titleEnglish: text(row?.title?.english), titleChinese: null, originalTitle: text(row?.title?.romaji), coverUrl: text(row?.coverImage?.extraLarge) ?? text(row?.coverImage?.large) ?? text(row?.coverImage?.medium), bannerUrl: text(row?.bannerImage) ?? text(row?.coverImage?.extraLarge), synopsis: text(row?.description), animeType: text(row?.format), broadcastStatus: text(row?.status), episodes: number(row?.episodes), episodeDuration: number(row?.duration), releaseYear: number(row?.seasonYear) ?? number(row?.startDate?.year), season: text(row?.season)?.toLowerCase() ?? null, startDate: anilistDate(row?.startDate), endDate: anilistDate(row?.endDate), ageRating: null, sourceMaterial: text(row?.source), publicScore: number(row?.averageScore) === null ? null : (number(row?.averageScore) ?? 0) / 10, genres: Array.isArray(row?.genres) ? row.genres.filter((value: unknown): value is string => typeof value === "string") : [], studios: values(row?.studios?.nodes), relations: Array.isArray(row?.relations?.edges) ? row.relations.edges.map((edge: any) => ({ relation: relationLabels[text(edge?.relationType) ?? ""] ?? "關聯作品", malId: number(edge?.node?.id) ?? 0, title: text(edge?.node?.title?.romaji) ?? text(edge?.node?.title?.english) ?? text(edge?.node?.title?.native) ?? "未命名作品", type: text(edge?.node?.type) })).filter((relation: AnimeRelation) => relation.malId > 0) : [], isAdult: Boolean(row?.isAdult), contentRating: row?.isAdult ? "成人內容" : null, externalUrl: text(row?.siteUrl), malId: number(row?.idMal), titleUserPreferred: text(row?.title?.userPreferred), synonyms: Array.isArray(row?.synonyms) ? row.synonyms.map(text).filter((value: string | null): value is string => Boolean(value)) : [], popularity: number(row?.popularity) };
}
...
export type AnimeSearchPage = { items: ExternalAnime[]; page: number; hasNextPage: boolean; total: number };

export async function searchAnimePage(query: string, page: number, perPage: number, callerSignal?: AbortSignal): Promise<AnimeSearchPage> {
  try {
    ...
    const items = await localizeAnimeTitles(media.map(mapAniList).filter((anime) => anime.id && !anime.isAdult));
    const info = payload?.data?.Page?.pageInfo;
    return { items, page: Number(info?.currentPage ?? page), hasNextPage: Boolean(info?.hasNextPage), total: Number(info?.total ?? items.length) };
  } catch (caught) {
    if (callerSignal?.aborted) throw caught;
    const failure = caught instanceof AnimeProviderError ? caught : new AnimeProviderError("anilist", 502, "unknown", caught instanceof Error ? caught.message : "AniList search failed");
    throw toSearchError([failure]);
  }
}
