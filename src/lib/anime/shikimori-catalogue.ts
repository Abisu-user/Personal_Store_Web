...
const SHIKIMORI_ROOT = (
  process.env.ANIME_SHIKIMORI_API_URL || "https://shikimori.one/api"
).replace(/\/+$/, "");
...
type ShikimoriRow = {
  ...
  score?: unknown;
  rating?: unknown;
  airedOn?: { date?: unknown };
  releasedOn?: { date?: unknown };
  poster?: { originalUrl?: unknown; mainUrl?: unknown };
...
function mapShikimori(row: ShikimoriRow, isAdult: boolean): ExternalAnime {
  ...
  return {
    id: String(row?.id ?? ""),
    source: "jikan",
    title: japanese ?? english ?? text(row?.name) ?? "未命名動漫",
    titleJapanese: japanese,
    titleEnglish: english ?? text(row?.name),
    titleChinese: null,
    originalTitle: text(row?.name),
    coverUrl: text(row?.poster?.originalUrl) ?? text(row?.poster?.mainUrl),
    bannerUrl: text(row?.poster?.originalUrl),
    synopsis: null,
    ...
    broadcastStatus:
      ...
      )[String(row?.status ?? "")] ?? text(row?.status),
    episodes: number(row?.episodes),
    episodeDuration: null,
    releaseYear: startDate ? Number(startDate.slice(0, 4)) || null : null,
    season: null,
    startDate,
    endDate: text(row?.releasedOn?.date)?.slice(0, 10) ?? null,
    ageRating: text(row?.rating) ?? (isAdult ? "Rx" : null),
    sourceMaterial: null,
    publicScore: number(row?.score),
    ...
    relations: [],
    isAdult,
    contentRating: isAdult ? "成人內容（18+）" : null,
    externalUrl: `https://shikimori.one/animes/${row?.id}`,
  };
}
  ...
  const order =
    filters.sort === "SCORE_DESC"
      ...
      : filters.sort === "START_DATE_DESC" ||
          filters.sort === "NEXT_AIRING_EPISODE_DESC"
        ? "aired_on"
        : filters.sort === "TITLE_ROMAJI"
          ? "name"
          : "popularity";
  const season = filters.seasonYear
    ? filters.season
      ? `${({ WINTER: "winter", SPRING: "spring", SUMMER: "summer", FALL: "fall" } as const)[filters.season]}_${filters.seasonYear}`
      : String(filters.seasonYear)
    : undefined;
  const args = [
    argument("page", page),
    argument("limit", perPage),
    `order: ${order}`,
    argument("rating", filters.includeAdult ? "rx" : undefined),
    argument("censored", !filters.includeAdult),
    argument("search", filters.search),
    argument("kind", filters.format?.toLocaleLowerCase()),
    argument(
      "status",
      filters.status
        ? (
...
export async function getShikimoriCatalogue(
  ...
  const response = await fetch(`${SHIKIMORI_ROOT}/graphql`, {
    ...
    headers: {
      ...
      "User-Agent": "Personal-Vault/1.0 (catalogue fallback)",
    },
  ...
  const payload = (await response.json().catch(() => null)) as {
    data?: { animes?: ShikimoriRow[] };
    errors?: Array<{ message?: unknown }>;
  } | null;
  if (!response.ok || payload?.errors?.length)
    throw new Error(`Shikimori catalogue returned ${response.status}`);
  const rows = Array.isArray(payload?.data?.animes) ? payload.data.animes : [];
  let items = rows
    .map((row) => mapShikimori(row, Boolean(filters.includeAdult)))
  ...
  if (filters.tag)
    items = items.filter((anime) =>
      anime.genres.some((genre) =>
        genre.toLocaleLowerCase().includes(filters.tag!.toLocaleLowerCase()),
      ),
    );
  const localized = filters.includeAdult
    ? await enrichAdultCatalogue(items)
    : await localizeAnimeTitles(items);
  return {
    items: localized,
    page,
    hasNextPage: rows.length === perPage,
    total: (page - 1) * perPage + rows.length,
    totalExact: rows.length < perPage,
  };
}
