...
export type AnimeSourceAvailabilityStatus =
  "available" | "not_found" | "unknown" | "checking" | "error" | "source_unavailable";
...
export type AnimeLibraryItem = {
  ...
  rating: number | null;
  favorite: boolean;
  personalRank: AnimePersonalRank | null;
  notes: string | null;
  startedWatchingAt: string | null;
...
export type AnimePreferences = {
  ...
  adultHiddenByDefault: boolean;
  adultAccessMode: AdultAccessMode;
  blurAdultCovers: boolean;
};
export type AnimeAdultPermissions = {
  adultContentAccess: boolean;
  adultContentAdmin: boolean;
};
export type AnimeWorkspaceData = {
  library: AnimeLibraryItem[];
  tags: AnimeTag[];
  folders: AnimeFolder[];
  logs: AnimeWatchLog[];
  preferences: AnimePreferences;
  adultPermissions: AnimeAdultPermissions;
};
...
export const animeStatusLabels: Record<AnimeWatchStatus, string> = {
  planning: "想看",
  watching: "正在觀看",
  completed: "已看完",
  paused: "暫停",
