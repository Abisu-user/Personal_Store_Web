...
export function normalizeAppearance(value: unknown): Appearance {
  ...
  const backgroundImages = (Array.isArray(candidate.backgroundImages) ? candidate.backgroundImages : legacyImage ? [legacyImage] : []).filter((item): item is string => typeof item === "string" && (item.startsWith("data:image/") || isStoredImage(item))).slice(0, 4);
...
export function applyAppearance(appearance: Appearance) {
  ...
  root.dataset.theme = normalized.theme === "system" ? (window.matchMedia("(prefers-color-scheme: dark)").matches ? "dark" : "light") : normalized.theme; root.dataset.accent = normalized.accent; root.dataset.background = normalized.background; root.dataset.density = normalized.density;
