export const appearanceStorageKey = "personal-vault:appearance:v4";
...
export type Appearance = {
  ...
  backgroundTint?: string; backgroundOverlay: boolean; backgroundBrightness: number; backgroundRotation: BackgroundRotation; backgroundRotationMinutes: number;
};

export const appearanceDefaults: Appearance = { theme: "system", accent: "blue", background: "paper", density: "comfortable", customColor: "#2b65bd", backgroundImages: [], backgroundActiveIndex: 0, backgroundPositionX: 50, backgroundPositionY: 50, backgroundZoom: 100, backgroundTint: "#FFFFFF", backgroundOverlay: false, backgroundBrightness: 100, backgroundRotation: "manual", backgroundRotationMinutes: 15 };
...
export function normalizeAppearance(value: unknown): Appearance {
  ...
  return { theme: candidate.theme as Theme, accent: candidate.accent as Accent, background: candidate.background as Background, density: candidate.density as Density, customColor: normalizeHexColor(candidate.customColor), backgroundImage: backgroundImages[candidate.backgroundActiveIndex ?? 0], backgroundImages, backgroundActiveIndex: Math.floor(clamp(candidate.backgroundActiveIndex, 0, Math.max(0, backgroundImages.length - 1), 0)), backgroundPosition: positions.includes(legacyPosition as typeof positions[number]) ? legacyPosition : "center", backgroundPositionX: clamp(candidate.backgroundPositionX, 0, 100, position[0]), backgroundPositionY: clamp(candidate.backgroundPositionY, 0, 100, position[1]), backgroundZoom: clamp(candidate.backgroundZoom, 100, 180, 100), backgroundTint: normalizeHexColor(candidate.backgroundTint, "#FFFFFF"), backgroundOverlay: candidate.backgroundOverlay === true, backgroundBrightness: clamp(candidate.backgroundBrightness, 60, 150, 100), backgroundRotation: rotations.includes(candidate.backgroundRotation as BackgroundRotation) ? candidate.backgroundRotation as BackgroundRotation : "manual", backgroundRotationMinutes: Math.round(clamp(candidate.backgroundRotationMinutes, 1, 1440, 15)) };
}

export function readAppearance(): Appearance { try { return normalizeAppearance(JSON.parse(window.localStorage.getItem(appearanceStorageKey) ?? window.localStorage.getItem("personal-vault:appearance:v3") ?? window.localStorage.getItem("personal-vault:appearance:v2") ?? window.localStorage.getItem("personal-vault:appearance:v1") ?? "{}")); } catch { return appearanceDefaults; } }
