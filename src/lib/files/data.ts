...
function logQueryError(query: string, error: SupabaseQueryError | null, level: "error" | "warn" = "error") {
  ...
  if (level === "warn") console.warn("[files:data] " + query + " failed", context);
  else console.error("[files:data] " + query + " failed", context);
}
...
export async function getFilesWorkspaceData(ownerId: string): Promise<FilesWorkspaceData> {
  ...
  const cleanupResult = await admin.from("entries").delete().eq("owner_id", ownerId).eq("kind", "file").lt("deleted_at", new Date(Date.now() - 30 * 86400000).toISOString());
  ...
  if (queryErrors.some(([, error]) => error)) throw new Error("Unable to load files.");
