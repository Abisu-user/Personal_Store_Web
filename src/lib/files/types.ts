...
export type StoredFile = { id: string; title: string; description: string | null; originalFilename: string; mimeType: string; byteSize: number; favorite: boolean; pinned: boolean; archived: boolean; deletedAt: string | null; folder: Pick<FileFolder, "id" | "name" | "is_visible"> | null; folders: Pick<FileFolder, "id" | "name" | "is_visible">[]; coverImageUrl: string | null; category: Pick<FileCategory, "id" | "name"> | null; categories: Pick<FileCategory, "id" | "name">[]; tags: FileTag[]; updatedAt: string };
