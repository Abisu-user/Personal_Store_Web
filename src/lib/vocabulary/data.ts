...
export async function getVocabularyWorkspaceData(userId: string, includeTrash = false): Promise<VocabularyWorkspaceData> {
  ...
  const [meaningsResult, examplesResult, cardTagsResult, deckCardsResult] = cardIds.length ? await Promise.all([
    admin.from("vocabulary_meanings").select("*").in("card_id", cardIds).order("sort_order"), admin.from("vocabulary_examples").select("*").in("card_id", cardIds).order("created_at"), admin.from("vocabulary_card_tags").select("card_id,tag_id").in("card_id", cardIds), admin.from("vocabulary_deck_cards").select("deck_id,card_id").in("card_id", cardIds),
  ]) : [{ data: [] }, { data: [] }, { data: [] }, { data: [] }];
  if (meaningsResult.error || examplesResult.error || cardTagsResult.error || deckCardsResult.error) throw new Error("Vocabulary relationships unavailable");
  ...
  const examples = (examplesResult.data ?? []).map((row: any): VocabularyExample => ({ id: row.id, cardId: row.card_id, meaningId: row.meaning_id, sentence: row.sentence, reading: row.reading, translation: row.translation, source: row.source, notes: row.notes, isFavorite: row.is_favorite }));
  ...
  meanings.forEach((meaning) => cardById.get(meaning.cardId)?.meanings.push(meaning)); examples.forEach((example) => cardById.get(example.cardId)?.examples.push(example));
